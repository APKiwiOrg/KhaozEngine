...
- **Version skew.** A new client sending a game message to an OLD server is flagged there as malformed; consumers gate adoption on the existing `WorldClientConfig.ProtocolVersion` handshake. The frame cannot be misparsed as a move by an old server (the 18-byte guard).
