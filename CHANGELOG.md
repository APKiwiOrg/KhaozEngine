...
Introduces a full 3D physics seam (`KhaozEngine.Physics`) and a BepuPhysics v2 backend (`KhaozEngine.Physics.Bepu`); `CharacterMovement.Step` now collide-and-slides capsule-vs-mesh against an `IPhysicsWorld`, fixing the domed-prop flank-clipping limitation and enabling building interiors. BREAKING: the netcode movement ctors replace the old `WorldColliders?`/`WorldSurfaces?` pair with `IPhysicsWorld?`.

- Physics - new **`KhaozEngine.Physics`** package (in the `Foundation` umbrella): a dependency-free seam for a static physics world. `IPhysicsWorld` supports adding/removing static bodies (`StaticHandle`), a fixed-timestep `Step(dt)`, and spatial queries (raycast `RayHit`, sweep-capsule `SweepHit`, `ComputePenetration`). Value-type collision shapes: `PhysicsShape.Sphere`, `.Capsule`, `.Box`, `.Cylinder`, `.ConvexHull`, `.TriangleMesh`, `.Compound`. `Pose` (position + quaternion orientation, `Pose.At`/`Pose.AtY`) and `PhysicsMaterial` (restitution, friction, density). `QueryFilter` masks bodies by category/mask bits. The seam is render-free and dependency-free; the backend is opt-in.
- Physics.Bepu - new **`KhaozEngine.Physics.Bepu`** package (NOT in any umbrella; opt-in like `WorldStore.Sqlite`): `BepuPhysicsWorld : IPhysicsWorld` over BepuPhysics v2 (`2.4.0`, pure-managed Apache-2.0, no native libs). `BepuPhysicsWorld.Create(gravity?)` creates a single-threaded deterministic `Simulation`. Statics are added via `AddStatic(shape, pose, material)` / `RemoveStatic(handle)`; `Step(dt)` advances the simulation. Raycast (`Raycast`), sweep-capsule (`SweepCapsule`), and penetration (`ComputePenetration`) queries mirror the seam contract. NativeAOT note: publishing under NativeAOT requires an `rd.xml` with `BepuPhysics` `Dynamic=Required All` (desktop/server targets are fine; this is relevant only for a future iOS AOT reach).
- Locomotion - `CharacterMovement.Step` gains an `IPhysicsWorld?` parameter (before the optional `clampXz`). When non-null: the capsule footprint (`MoveTuning.CapsuleRadius`) performs a sweep-capsule query into the physics world for collide-and-slide against all static meshes (convex hull / triangle mesh shapes from `ke-propbake`), replacing the old 2D `WorldColliders` XZ push-out path; a downward capsule probe computes vertical support from prop tops, so a character stands cleanly on domed rocks and climbs their flanks rather than clipping through the slope. Terrain height stays analytic (the `TerrainCollision` delegate is unchanged). Null = terrain-only, identical to pre-8.0.0 behaviour. The 2D footprint `Step` overloads that accepted `WorldColliders?` and `WorldSurfaces?` are removed.
- Render3D / PropCollisionBake - `ke-propbake` now bakes a 3D collision shape per prop (convex hull for solid props, triangle mesh for buildings) into the kit manifest alongside the existing `.surf` walkable surface heightmap. `AssetEntry` gains `CollisionShape`; `PropCollisionLoader.Load(entry)` returns the baked `PhysicsShape`; `Scene3DChunkSink.LoadChunk`/`UnloadChunk` call `IPhysicsWorld.AddStatic`/`RemoveStatic` per prop so the streamer keeps the physics world in sync with the visible chunk ring.
