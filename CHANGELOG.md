...
- **Smooth angled stair climb (`KhaozEngine.Locomotion`).** The paced step-climb (`MaxStepClimbSpeed`) capped only the vertical rise, letting a step-up mount's horizontal teleport race ahead while the vertical lagged; with the forward then blocked by the next riser, the leftover horizontal speed leaked sideways along the tread and, on an off-axis approach, accumulated into the shaft wall and WEDGED the climb (and pulsed a fore-aft lurch even head-on). Two fixes: `CharacterMovement` now paces the WHOLE step-up displacement as a unit (scaling horizontal in lock-step with the capped vertical), and the step-up probe now climbs PERPENDICULAR to the riser edge (up the stairs) instead of along the raw, possibly-angled, move direction. Result: an angled climb ascends square to the treads and merely slides along the shaft wall (a normal flat-ground wall slide), reaching the top at every approach angle, with uniform forward speed and no sideways push - a stair feels like a ramp. Headless tests pin no fore-aft lurch and no lateral drift head-on, and no wedge or lateral amplification at 8 and 15 degrees off-axis.
- **Showcase demos adopt `CharacterAvatar` (`KhaozEngine.Showcase`).** `Room3D` and `RoomDungeon` drop their duplicated character glue (rig load, `IntendedMoveDirection`/`Wrap` facing, the speed-derive + animation feed, the feet-draw matrix) and drive a single `CharacterAvatar` instead, keeping the greybox-capsule fallback when the rig asset is absent.
