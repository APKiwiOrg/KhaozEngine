...
`InMemoryAccountStore` ships here. The durable backends are opt-in sibling packages in no umbrella:
`KhaozEngine.Accounts.Sqlite` (`SqliteAccountStore`) and `KhaozEngine.Accounts.SqlServer` (`SqlServerAccountStore`),
both on Grimhollow's flat `accounts` layout under a configurable table name, so that database is adopted in place.
