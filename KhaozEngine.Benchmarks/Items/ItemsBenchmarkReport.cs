...
public static class ItemsBenchmarkReport
{
    public static void Write(ItemsBenchmarkResult result, TextWriter output)
    {
        ...
        output.WriteLine(string.Create(culture, $"budget 5 detail: mean {result.Budget5MeanMicroseconds:F2} us, {result.Budget5AllocatedBytesPerGeneration:F0} bytes per generation, pool {result.Budget5MeanPoolSize:F0} candidates, memo hit rate {result.Budget5MemoHitRate:P1}, mean affixes {result.Budget5MeanAffixCount:F2}"));
        output.WriteLine(string.Create(culture, $"budget 5 cost shape: {result.Budget5CandidateVisitsPerGeneration:F0} candidate visits per generation at {result.Budget5NanosecondsPerCandidateVisit:F2} ns each"));
        output.WriteLine(string.Create(culture, $"budget 5 warm memo (64 bases, 4 item levels): p50 {result.Budget5WarmP50Microseconds:F2} us, p99 {result.Budget5WarmP99Microseconds:F2} us, {result.Budget5WarmAllocatedBytesPerGeneration:F0} bytes per generation, memo hit rate {result.Budget5WarmMemoHitRate:P1}"));
        ...
        output.WriteLine(string.Create(culture, $"budget 9 detail: GetTotalMemory delta {Megabytes(result.Budget9TableTotalMemoryDeltaBytes)} MB against a heap-size delta of {Megabytes(result.Budget9TableResidentBytes)} MB"));
