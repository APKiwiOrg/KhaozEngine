...
/// at least once while keeping a full <c>--replication</c> run under about two minutes in Release. Every row uses
/// the config's default warmup/timed tick counts, so per-tick figures are directly comparable across rows.
/// </summary>
public static class ReplicationBenchmarkMatrix
{
    ...
    public static IReadOnlyList<ReplicationBenchmarkConfig> Default() => new[]
    {
        ...
        new ReplicationBenchmarkConfig { Name = "C=64 E=16384 comp=1", ClientCount = 64, EntityCount = 16384, ComponentsPerEntity = 1 },
        new ReplicationBenchmarkConfig { Name = "C=64 E=16384 comp=4", ClientCount = 64, EntityCount = 16384, ComponentsPerEntity = 4 },
    };
