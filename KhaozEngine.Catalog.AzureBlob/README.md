...
foreach (string hash in clientClosure)
{
    ReadOnlyMemory<byte>? bytes = await local.GetAsync(hash);
    if (bytes is not null)
    {
        await origin.PutAsync(hash, bytes.Value);
    }
}
```
...
`GetAsync` answers null for an absent object rather than throwing, and refuses an object larger than
`HttpPackStore.MaxObjectBytes` from its DECLARED length, before a body is buffered.
...
Server-only chunks never go into a public container. Nothing in this package can enforce that, because a
store is handed a hash and bytes and has no way to know which side's manifest named them. The caller copies
the CLIENT manifest's closure ONLY: the client manifest, its remap rule chunk, its text chunks and its
client-visible type chunks. Copying a version's whole keep set, or pointing a publish at this store, would
put the server projection into a container anyone can read.

## The live leg

The test suite runs entirely against an in-memory double of the internal container seam, so it needs no
account, no network and no credential. One gated fact talks to a real container, skipped unless
`KE_PACKSTORE_BLOB` holds a container URL whose container NAME contains `-packstore-test-`. The leg deletes
what it writes, so the marker is checked before it writes a byte, the same rule the SQL Server legs apply to
