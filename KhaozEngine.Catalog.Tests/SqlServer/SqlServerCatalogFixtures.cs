...
/// Server or Azure SQL connection string. It is the same variable the world store's SQL Server conformance
/// reads, deliberately: one instance and one variable serve every SQL Server suite in the engine. The SQLite
/// backend carries the always-on coverage, CI has no SQL Server, so these run locally or against a test
/// database on demand.
/// </summary>
public sealed class SqlServerFactAttribute : FactAttribute
{
    ...
    {
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable(SqlServerCatalogDatabase.EnvironmentVariable)))
        {
            Skip = "set KE_SQLSERVER_TEST_CONNSTRING to run the SQL Server content catalog suite";
        }
    }
}
...
{
    /// <summary>The variable naming the instance, shared with the world store's SQL Server suite.</summary>
    internal const string EnvironmentVariable = "KE_SQLSERVER_TEST_CONNSTRING";
