...
{
    /// <summary>
    /// THE DEVICE'S ONE RING ALLOCATOR (M-M3, M-M5, section 9.2): the frame segment every uniform ring writes
    /// into, the completion gate that decides when a segment may be reused, the off-timeline write's
    /// pending-patch queue, and the backpressure MM4 measures.
    ///
    /// <para><b>SEGMENT ROTATION IS DEVICE-WIDE.</b> Frame N writes segment <c>N % FramesInFlight</c> in EVERY
    /// ring at once, so this type holds the index and each <see cref="MetalUniformRing"/> multiplies it by its
    /// own stride. One index rather than one per buffer is what makes a frame's uniforms consistent.</para>
    ///
    /// <para><b>AND THE FRAME BOUNDARY IS <c>MetalCommandList.Begin</c>, WHICH IS WHERE THIS BACKEND DIVERGES
    /// FROM BOTH SIBLINGS (M-R2).</b> On Direct3D 11 and on Vulkan the equivalent <c>BeginFrame</c> is called
    /// from <c>Present</c>, and each of those backends has a SECOND per-list index (a mapped-ring scope there, a
    ...
    /// <c>[GpuFact]</c> run: the ring would never rotate, every frame would write segment 0, and the gate this
    ...
    /// <c>signaledValue</c>, and blocks while the GPU has not reached the value that segment's frame was closed
    /// at. A ring gated on a submit RECEIPT would recycle a segment the moment the CPU finished ASKING for the
    /// work rather than when the GPU finished doing it, which is a silent, intermittent corruption several frames
    /// from its cause. That dependency is why this row waits on row 5
    /// (https://github.com/APKiwiOrg/KhaozEngine/issues/571).</para>
    ///
    /// <para><b>AND THE OWNER VALUE IS <see cref="MetalTimeline.LastSubmitted"/> READ UNDER THE SUBMIT
    /// LOCK.</b> A submission allocates its value, encodes the signal and registers it inside that one lock
    /// (M-N2), so there is no submission in the window between the two: every submission ever made has either
    /// registered at or below this value or FAILED. It is <see cref="MetalTimeline.LastSubmitted"/> rather than
    /// <see cref="MetalTimeline.LastAllocated"/> for one reason in one direction: a submit that threw between the
    /// allocation and the commit took a value nothing will ever signal, so gating a segment on the allocation
    /// high-water would block that segment forever.</para>
    ///
    ...
    /// <see cref="BeginFrame"/> the only member here that can block, and what makes a caller who already holds
    ...
    internal sealed class MetalRingAllocator
    {
        ...
        // The timeline value each segment's frame was CLOSED at, which is at or above every value a submission
        // made while that segment was current took and had accepted. Zero means the segment has never been closed
        // with anything submitted, which is the only case that needs no wait: the timeline's first value is 1, so
        // zero cannot be a real target.
        ...
        // recording built across a frame boundary already has.
        int _segment;
        ...
        /// to order <c>-commit</c> and the timeline value it encodes.</param>
        ...
        /// <summary>
        /// Segment acquisitions that ACTUALLY BLOCKED, since the device was created. The same entries are
        /// recorded into <see cref="MetalBackpressure"/>, which is what the seam reports, and this is the ring's
        /// own half of that reading so a test can say which source produced a count. On this backend they are the
        /// same number, because the accumulator has no second source (M-R2).
        /// </summary>
        ...
        /// <summary>
        /// CLOSE THE FRAME JUST BUILT AND OPEN THE NEXT ONE: record what the outgoing segment has to be waited
        /// for, advance, block there if the GPU has not finished with it, replay anything queued for it, and
        /// publish it. Returns the segment now current, which is what the caller's staging arena rotates onto.
        /// <para>
        ...
        /// </para>
        /// <para>
        /// CALL IT WITHOUT THE SUBMIT LOCK, AND A CALLER HOLDING IT IS REFUSED BY NAME. This is the one member
        /// here that can BLOCK: the gate waits until the GPU has finished with the segment being opened, which is
        /// up to a frame. Under the lock that would be a frame-long hold of the one serialised point in the
        /// frame, and it would also shut out the submission that would end the wait.
        /// </para>
        /// <para>
        /// THE THREE STEPS ARE IN THIS ORDER FOR A REASON. The outgoing segment's owner is recorded FIRST, under
        /// the lock, so it names every submission that could have referenced it. The wait happens SECOND, outside
        /// the lock. The patch replay and the publish happen THIRD and in ONE hold, because an off-timeline write
        /// that observes the new segment as current copies into it directly, so observing that before the replay
        /// would have its bytes overwritten by older queued ones. Draining and publishing together leaves a
        /// concurrent writer only two orderings and both are correct.
        /// </para>
        /// </summary>
        internal int BeginFrame()
        {
            if (Monitor.IsEntered(_submitLock))
            {
                throw new InvalidOperationException(
                    "BeginFrame was called on the native Metal ring allocator while the caller held the submit "
                    + "lock. Opening a frame waits for the GPU to finish with the segment it opens, which is up "
                    + "to a frame, and that lock is the one serialised point in the frame (M-N2). Begin takes it "
        ...
        /// byte range and a private copy of the data are queued on the ring, and the <see cref="BeginFrame"/>
        /// that next opens that segment applies them, right after the gate has proved the GPU is done with it.
        ...
        /// submitted and is skipped without a poll at all, so a load-time write, when every owner is zero, costs
        /// no completion read whatsoever.
        /// </para>
        /// <para>
        ...
        /// patch still queued would let the frame boundary replay the OLDER bytes over the newer ones.
        /// </para>
        /// <para>
        ...
        /// submit and its next acquire is a whole wrap away.
        /// </para>
        /// </summary>
        ...
        /// existing, so a frame boundary replaying it after the <c>MTLBuffer</c> was released would write through
        /// a dangling <c>contents()</c> pointer. Objective-C reference counting keeps the ALLOCATION alive while
        ...
        // lock, which is what makes LastSubmitted exact: a submission allocates and registers its value inside
        // that same lock, so there is no submission in flight between the two while this runs.
        ...
        // THE BACKPRESSURE WAIT, and MM4's only source. The poll first, then the block, and only the block is
        // recorded: a counter that ticks on a non-wait cannot answer "was anything ever blocked" with a zero.
        //
        // It blocks on the shared event rather than spinning, like the Vulkan sibling and unlike the Direct3D 11
        // one, and the difference is the primitive rather than a preference. There a completion poll on the
        // fallback mechanism re-enters the submit lock, so a sleeping wait would shut out the submission that
        // would end it. Here the wait holds no lock at all and waitUntilSignaledValue:timeoutMS: is a real
        // blocking form, so burning a core to avoid a syscall would trade the one resource a stalled frame still
        // needs.
        void AcquireSegment(int segment)
        {
            ...
            // A dead device answers its own last allocated value here, which is at or above anything a segment
            // can hold, so this returns without waiting rather than blocking on a counter nothing can advance.
        ...
        // though it is free, because a direct copy would be overwritten by the older queued bytes when the
        // boundary replays them.
        ...
        // Apply the segment's pending patches and THEN publish it as current, in ONE hold. See BeginFrame's
        // remarks for why the two steps are one critical section. It walks the patched rings alone, not every
        // ring in the device, and a ring leaves that registry as soon as it owes nothing anywhere.
