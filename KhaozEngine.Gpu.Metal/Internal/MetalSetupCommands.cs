...
namespace KhaozEngine.Gpu.Metal.Internal
{
    ...
    /// (https://github.com/APKiwiOrg/KhaozEngine/issues/573) added the third site by flushing at the top of
    /// <c>IGpuDevice.Submit</c>, outside that method's own lock.</para>
    ///
    ...
    internal sealed class MetalSetupCommands : IDisposable
    {
        ...
        /// It is a reading of the very command buffer the uploads were encoded into, so a test that has it is
        /// asserting something about the eleven-argument copy rather than about the process not having aborted.
        /// It is no longer the ONLY such reading: the command-list row
        /// (https://github.com/APKiwiOrg/KhaozEngine/issues/573) registers the device's queue with
        /// <see cref="MetalCompletionHandler"/>, so the handler attached at <c>BeginBatch</c> now delivers a
        /// failed setup batch to the device's loss latch on the shipping path, by the same route a failed frame
        /// takes. This stays because a test wants the outcome of ONE named batch rather than the device's
        /// latched state.</para>
        ///
        ...
        ///
        /// <para><b>THE OBLIGATION THIS LEFT FOR THE COMMAND-LIST ROW IS MET</b>
        /// (https://github.com/APKiwiOrg/KhaozEngine/issues/573): <c>IGpuDevice.Submit</c> calls this BEFORE
        ...
        ///
        /// <para><b>IT REPORTS WHETHER IT COMMITTED, AND THAT ANSWER IS LOAD-BEARING RATHER THAN INFORMATIONAL.</b>
        /// A batch committed here signals NO value on the device's timeline, so M-F5's counted drain is blind to
        /// it, and the flush has just put it BEHIND everything that drain can see. The device's own drain point
        /// takes a second, queue-level drain when and only when this returned true. <see cref="FlushCount"/> is
        /// the same reading as a running total, and it is not what the drain reads, because a caller comparing it
        /// before and after would be reading a counter another thread can move between the two reads.</para>
        /// </summary>
        /// <returns>True when a batch was open and has now been committed.</returns>
        internal bool Flush()
        {
            lock (_gate)
            {
        ...
        // releases the staging buffers the batch accumulated. True when something was committed to the queue,
        // which is what the caller's drain decision reads (see Flush).
        bool CommitOpenBatch()
        {
            ...
            if (_liveness.IsDead)
            {
                ...
                // FALSE, because nothing reached the queue. A caller that drained on this answer would commit an
                // empty buffer to a queue whose device has gone, which is the one call this arm exists to avoid.
                return false;
            }
