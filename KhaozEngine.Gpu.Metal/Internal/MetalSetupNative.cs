...
namespace KhaozEngine.Gpu.Metal.Internal
{
    ...
    internal sealed class MetalSetupNative : IMetalSetupNative
    {
        ...
        /// a frame can. It is LIVE now: device creation registers the queue with
        /// <see cref="MetalCompletionHandler"/> (the command-list row's wiring,
        /// https://github.com/APKiwiOrg/KhaozEngine/issues/573), and the handler is one global block keyed on the
        /// buffer's own queue, so a failed setup batch reaches the device's latch by exactly the route a failed
        /// frame does. A completion arriving after teardown finds the slot cleared and is dropped, which is the
        /// direction that path is meant to fail in.
        /// </para>
        /// </remarks>
