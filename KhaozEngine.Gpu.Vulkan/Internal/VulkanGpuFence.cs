...
{
    /// <summary>
    /// The seam's <see cref="IGpuFence"/> on the native Vulkan backend: a REMEMBERED VALUE on the device's one
    /// <see cref="VulkanTimeline"/>, and no device object of its own.
    /// <para>
    /// THE WHOLE TYPE IS THAT SENTENCE. A timeline semaphore gives a device one monotonic counter rather than a
    /// fence per submission, so "has submission N finished" is answered by comparing that counter against the
    /// value submission N signalled. Every fence the seam hands out is therefore two fields and a comparison, and
    /// the expensive object is created once per device. There is no <c>VkFence</c> anywhere in this backend.
    /// </para>
    /// <para>
    /// THE LIFECYCLE. A fence is UNARMED when created, which reads unsignalled, because the seam requires a fence
    /// to be unsignalled when it is submitted. <c>Submit</c> arms it with the value that submission was allocated,
    /// and from then on <see cref="Signaled"/> is that value against the timeline's completed value.
    ...
    /// </para>
    /// <para>
    /// AFTER DEVICE DEATH IT READS TRUE (V-F10), and that wins over everything else including an unarmed fence. A
    /// destroyed device has no outstanding work, so "is it done" is yes, and answering no is what would strand a
    ...
    /// </para>
    /// </summary>
    internal sealed class VulkanGpuFence : IGpuFence
    {
        readonly VulkanTimeline _owner;

        // 0 is "unarmed". The timeline semaphore is created at 0 and the first submission takes 1, so no real
        // target can collide with it.
        ulong _target;
        bool _disposed;

        internal VulkanGpuFence(VulkanTimeline owner)
        ...
        public bool Signaled
        {
            get
            {
                // Death first, and it wins over the unarmed case too (V-F10). See the class note.
        ...
        /// Called by the submit path (row 7, https://github.com/APKiwiOrg/KhaozEngine/issues/517) and by nothing
        /// else.
        /// </summary>
        internal void Arm(ulong target)
        {
            ...
            if (target == 0)
            {
                throw new ArgumentOutOfRangeException(nameof(target),
                    "A native Vulkan fence was armed with timeline value 0, which is the unarmed marker. The "
                    + "device timeline is created at 0 and the first submission takes 1, so reaching here means a "
                    + "fence was armed without a value having been allocated.");
            }

            if (_target != 0)
            {
                throw new InvalidOperationException(
                    "A native Vulkan fence was submitted while it was still armed from an earlier submission. The "
