...
namespace KhaozEngine.Gpu.Vulkan.Internal
{
    ...
    /// <see cref="VulkanRingAllocator.UpdateBuffer"/> for a segment the GPU has not finished with, and applied by
    /// the <see cref="VulkanRingAllocator.BeginFrame"/> that next opens that segment.
    /// <para>
    ...
    /// </para>
    /// </summary>
    internal readonly struct VulkanRingPatch
    {
        internal VulkanRingPatch(ulong offsetBytes, byte[] data)
        {
        ...
        internal ulong OffsetBytes { get; }

        /// <summary>What was written, copied out of the caller's span at record time.</summary>
        internal byte[] Data { get; }

        /// <summary>
        /// Whether this patch's range FULLY covers <paramref name="other"/>'s, which is the one case where dropping
        /// the older entry changes nothing: a later write that spans an earlier one entirely overwrites every byte
        /// of it, so replaying both and replaying only this one leave identical memory. A partial overlap is NOT
        /// coalesced, because there the older bytes outside the newer range still have to land.
        /// </summary>
        internal bool Covers(in VulkanRingPatch other)
            => OffsetBytes <= other.OffsetBytes
                && OffsetBytes + (ulong)Data.Length >= other.OffsetBytes + (ulong)other.Data.Length;
    }

    /// <summary>
    /// THE PENDING PATCHES OF ONE RING, per segment and in arrival order. This is what an off-timeline write leaves
    /// behind INSTEAD OF WAITING, and it is the whole of the mechanism V-M8 adopts wholesale from
    /// https://github.com/APKiwiOrg/KhaozEngine/issues/484 rather than re-deriving.
    /// <para>
    ...
    /// fully covers, so the repeated case (one caller rewriting the same range off-timeline) stays at one entry per
    /// segment forever rather than growing without limit. Beyond that the bound is the workload's: an off-timeline
    /// uniform write is a LOAD-TIME or settings-change write by construction, because the per-frame ones go through
    /// the record-time path, which touches the current segment alone.
    /// </para>
    /// <para>
    /// NOT THREAD-SAFE, and it does not need to be. Every mutation and every read happens under the device's submit
    /// lock, on both the recording side (<see cref="VulkanRingAllocator.UpdateBuffer"/>) and the applying side
    /// (<see cref="VulkanRingAllocator.BeginFrame"/>).
    /// </para>
    /// </summary>
    internal sealed class VulkanRingPendingPatches
    {
        readonly List<VulkanRingPatch>[] _bySegment;
        int _pending;

        internal VulkanRingPendingPatches(int framesInFlight)
        {
            _bySegment = new List<VulkanRingPatch>[framesInFlight];
            for (int i = 0; i < framesInFlight; i++) _bySegment[i] = new List<VulkanRingPatch>();
        }
        ...
        /// <summary>How many patches are outstanding across every segment. A diagnostic number and the thing a test
        /// asserts the coalescing rule against.</summary>
        ...
        /// <summary>Whether one segment has anything outstanding. The off-timeline write asks this before it copies
        /// directly: a segment with a patch queued takes the queue for every later write too, so the two cannot
        /// land out of order.</summary>
        ...
        /// in.</summary>
        internal IReadOnlyList<VulkanRingPatch> ForSegment(int segment) => _bySegment[segment];
        ...
        /// </summary>
        internal int Record(int segment, ulong offsetBytes, ReadOnlySpan<byte> data)
        {
            var patch = new VulkanRingPatch(offsetBytes, data.ToArray());
            List<VulkanRingPatch> list = _bySegment[segment];
        ...
        /// patches name memory that is about to stop existing, so carrying them would be a write through a mapping
        /// whose chunk has been freed at the next frame boundary.
        /// <para>
