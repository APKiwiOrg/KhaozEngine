...
- **Texture arrays, including one-layer ones** - `GpuTextureDescription.Texture2DArray(w, h, format, usage,
  ...
  2D-array case only: a cubemap (`GpuTextureUsage.Cubemap`) and a multisampled texture keep their own type rules.
  The three native backends create the array type directly. The incumbent Veldrid backend cannot express a
  one-layer array at all, so it creates a second, never-addressed slice and keeps the seam's logical layer count
  at one.
