...
public static class AuthExchangeHosting
{
    ...
    /// exchange endpoint and anything else that reads the client address.
    /// </summary>
    ...
    public static IApplicationBuilder UseAuthExchangeHosting(this IApplicationBuilder app)
    {
        ...
        if (app.ApplicationServices.GetService<HostingMarker>() is null)
    ...
    // Registered by AddAuthExchangeHosting so the Use call can tell a composition that skipped it.
    internal sealed record HostingMarker(AuthExchangeHostingOptions Options);
}
