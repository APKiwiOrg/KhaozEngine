...
public enum EditorToolMode
{
    ...
    DrawScatterOverride,
    /// <summary>Sculpt the terrain's authored height deltas with a brush (raise / lower / smooth / flatten /
    /// set-height, chosen in the inspector). A press-drag-release stroke is one undo step. Appended last so the
    /// toolbar cast keeps every prior mode's index.</summary>
    SculptTerrain,
}
...
public sealed partial class EditorToolController
{
    ...
    public string ModeHint => _mode switch
    {
        ...
        EditorToolMode.SculptTerrain => "Sculpt terrain. Drag to sculpt height, pick the brush and radius in the inspector.",
        _ => _mode.ToString(),
    };
    ...
    public void Update(in EditorFrameInput input)
    {
        ...
        switch (_mode)
        {
            ...
            case EditorToolMode.SculptTerrain: UpdateSculpt(input); break;
        }
    }
