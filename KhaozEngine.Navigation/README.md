...
replan cooldown. `ActivePath` is the committed `NavPath` (the same immutable instance the planner returned,
so it is a read-only view with no path back into follower state), and `ActiveWaypointIndex` is the index of
the waypoint currently being steered to, so `ActivePath.Waypoints[ActiveWaypointIndex]` onward is the
...
`Reset`, once the goal is reached (`Arrived`), and while the goal is `Unreachable`. While a replan is merely
due but still gated by `ReplanCooldownSeconds`, it stays the previously committed path, so you always read
the route the agent is steering on, never a mid-cooldown re-plan. When non-null it always carries at least
one waypoint and `ActiveWaypointIndex` is a valid index into its `Waypoints` (zero, and not meaningful, when
`ActivePath` is `null`).
