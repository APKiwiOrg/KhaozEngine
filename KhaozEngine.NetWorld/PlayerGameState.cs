...
public readonly struct PlayerPersistenceContext
{
    ...
    /// <summary>The durable key the player's record is keyed by (stable across sessions and cell handoffs).
    ...
    /// <summary>The verified account subject used for authentication and live-session policy.</summary>
    public string AuthenticatedAccountId { get; }

    /// <summary>Constructs a context whose durable key and authenticated account id are the same value.</summary>
    public PlayerPersistenceContext(int slot, string accountId)
        : this(slot, accountId, accountId)
    {
    }

    /// <summary>Constructs a context with separate durable and authenticated identities.</summary>
    public PlayerPersistenceContext(int slot, string accountId, string authenticatedAccountId)
    {
        Slot = slot;
        AccountId = accountId;
