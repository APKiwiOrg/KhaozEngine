...
namespace KhaozEngine.Tests.Gpu
{
    ...
    public sealed class D3D11EmitterResolutionTests
    {
        ...
        /// <summary>
        /// Every Direct3D 11 scissor slot is accepted and the first index past the API limit is refused by the
        /// state both emitters share.
        /// </summary>
        [Fact]
        public void ScissorIndicesUseTheDirect3D11LimitForBothEmitters()
        {
            var emitter = new D3D11NativeTraceEmitter(new D3D11DeviceState(), new D3D11NativeCallLog());
            emitter.Begin();

            emitter.SetScissorRect(D3D11ScissorState.MaximumCount - 1, 0, 0, 8, 8);
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                emitter.SetScissorRect(D3D11ScissorState.MaximumCount, 0, 0, 8, 8));
        }
