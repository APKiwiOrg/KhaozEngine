...
namespace KhaozEngine.Tests.Gpu
{
    ...
    public sealed class D3D11NativeCallBudgetTests
    {
        ...
        /// layout contributes one constant buffer, eight shader resources and two samplers, so the drained set
        /// lands at <c>b1 t8 s2</c>. Under the tail pipeline it would be <c>b0 t0 s0</c>, and in fact it would not
        ...
        public void Invariant_AMidFramePipelineSwitchDrainsUnderTheOutgoingLayouts()
        {
            ...
            Assert.Equal(
                ...
                harness.Log.Trace[0]);
            Assert.StartsWith("PSSetShaderResources(8,1,", harness.Log.Trace[1], StringComparison.Ordinal);
