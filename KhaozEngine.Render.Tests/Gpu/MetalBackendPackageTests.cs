...
namespace KhaozEngine.Tests.Gpu
{
    ...
    public sealed class MetalBackendRegistrationTests
    {
        ...
        /// <para>
        /// Row 2 registers under <c>(GpuBackendKind)6</c> because row 3
        /// (https://github.com/APKiwiOrg/KhaozEngine/issues/569) owns the <c>MetalNative = 6</c> append and
        /// parallelises with this row rather than preceding it. The registry keys by VALUE, so the registration
        /// is correct today and becomes correct BY NAME the moment that member lands. What must not happen is
        /// the cast surviving the append, because a magic number nobody is forced to remove is how a temporary
        /// shim becomes permanent.
        /// </para>
        /// <para>
        /// So this row fails the moment ordinal 6 gains a name, and its message is the instruction. If you are
        ...
        {
            Assert.Equal(6, (int)KhaozEngineMetal.MetalNativeKind);

            Assert.False(Enum.IsDefined(typeof(GpuBackendKind), KhaozEngineMetal.MetalNativeKind),
                "GpuBackendKind ordinal 6 now has a name, so row 3's append has landed and the shim it was "
                + "written around is done. Two edits and this is finished: replace the (GpuBackendKind)6 cast on "
                + "KhaozEngineMetal.MetalNativeKind with GpuBackendKind.MetalNative (or drop the constant and "
