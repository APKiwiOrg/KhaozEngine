...
namespace KhaozEngine.Tests.Gpu
{
    ...
    ///
    /// <para>
    /// DEDUPLICATED BY SOURCE PAIR, not by call site. The catalog contains 56 graphics pairs and eight compute
    ...
    public static class ShippedShaderPrograms
    {
        ...
        /// <summary>Every distinct shipped vertex and fragment pair, 56 of them.</summary>
        public static IEnumerable<ShippedGraphicsProgram> GraphicsPrograms()
        {
            ...
            // The alpha-cutout pair (issue #15), the only depth fragments that sample a texture, and the
            // dissolve-aware skinned vertex (issue #387), which reuses the rigid dissolve fragment unchanged.
            yield return new("ShadowDepthCutout",
                ShaderSources.ShadowDepthCutoutVert, ShaderSources.ShadowDepthCutoutFrag);
            yield return new("ShadowDepthCutoutInverted",
                ShaderSources.ShadowDepthCutoutVert, ShaderSources.ShadowDepthCutoutInvertedFrag);
            yield return new("SkinnedShadowDepthDissolve",
                ShaderSources.SkinnedShadowDepthDissolveVert, ShaderSources.ShadowDepthDissolveFrag);

            // Render3D point-light shadow atlas, the same three caster variants one geometry over plus the
            // per-row clear quad. The casters carry the same input sink the cascade ones do, for the same
            // FXC/WARP reason. The clear pair declares no vertex input at all (it is a fullscreen triangle off
            // gl_VertexIndex), which is why the vertex-layout sweep skips it and the hash tables do not.
            yield return new("PointShadowRigid",
                ShaderSources.PointShadowRigidVert, ShaderSources.PointShadowRigidFrag);
            yield return new("PointShadowRigidDissolve",
                ShaderSources.PointShadowRigidDissolveVert, ShaderSources.PointShadowRigidDissolveFrag);
            yield return new("PointShadowRigidDissolveInverted",
            ...
            yield return new("PointShadowClear",
