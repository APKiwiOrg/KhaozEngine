...
{
    /// <summary>
    /// Minimal, dependency-free PNG encoder for 8-bit RGBA buffers (the layout
    /// <see cref="Render2DSnapshot.Capture"/> returns). Uses only the BCL (<see cref="ZLibStream"/> for the
    /// IDAT zlib stream + a CRC-32 table), so the Render2D package gains no image-library dependency. Tooling /
    /// test helper - not a general image library (no palette, interlace, or non-RGBA support).
    /// </summary>
