...
namespace KhaozEngine.Render3D
{
    ...
    /// frame (the industry-default reference), and rotation deltas are BOTH extracted and applied in the joint's LOCAL
    /// frame (<c>delta = inverse(reference) * sample</c>, applied as <c>base * delta</c>, the Unity/Unreal/glTF-additive
    /// convention), so a base equal to the reference reproduces the authored sample exactly.</para>
    ///
    ...
    public sealed class LayeredAnimator
    {
        ...
        // The per-node additive math and its convention (#20) live in PoseBlend.ApplyAdditive, the one implementation
        // PoseBlend.AddInto shares.
        void CompositeAdditive(int nodes, AnimationLayer layer, float layerWeight)
        {
            ...
            for (int n = 0; n < nodes; n++)
            {
                float w = mask is null ? layerWeight : layerWeight * mask.Weight(n);
                if (w <= 0f) continue;
                _composited[n] = PoseBlend.ApplyAdditive(_composited[n], _layerScratch[n], reference[n], w);
            }
        }
