...
- Water surface grid (`WaterSettings.GridMode`, a `WaterGridMode`): `CameraFocused` (the default) is the fixed
  ...
  camera-relative `RenderOrigin`, so an origin rebase moves no ring. Since 17.1.0 `ClipmapGeomorphBand` (0.5, `0`
...
- Bathymetry, shoaling and breaking surf (since 17.1.0, `WaterSettings.Bathymetry`, **null by default = the
