...
namespace KhaozEngine.Render3D.Rendering
{
    ...
    internal sealed class OceanFftProducer : IDisposable
    {
        ...
        readonly record struct Bake(float WindSpeed, float WindDirection, float Fetch, float Depth,
            ...
            int Cascades, float Tile, float Ratio, int Resolution);
        ...
        /// </para>
        /// </summary>
        public IGpuTexture Map => _map ?? _idleMap!;
        ...
        /// 1 when active, 0 when not, independent of cascade count and resolution.</summary>
        ...
        public bool Update(IGpuCommandList sceneList, WaterSettings settings, float timeSeconds)
        {
            EnsureIdle();
            LastStallCount = 0;
            LastStallMs = 0d;
            ...
            var want = new Bake(sea.WindSpeed, sea.WindDirectionDegrees, sea.FetchKilometres, sea.DepthMetres,
                ...
                sea.Seed, cascades, sea.CascadeTileMetres, sea.CascadeTileRatio, resolution);
        ...
        {
            bool shapeChanged = !_hasBake || _baked.Resolution != want.Resolution || _baked.Cascades != want.Cascades;
