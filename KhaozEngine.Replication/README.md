...
- **`ReplicationRegistry`** - register each replicated component type with serialize/deserialize (and optional
  ...
  **`ReplicationRegistry.FirstExtensionTypeId`** (= 16, `IsExtension(id)`); ids `1..15` are reserved for engine
  ...
  id **skips** it (forward-compatible), while an unknown built-in id stays a hard mismatch. Since 17.39.0 an
  extension `deserialize` closure reads through a reader bounded to EXACTLY that component's own framed payload,
  so a codec that carries a declared length of its own can check it against `BaseStream.Length - Position` and a
  lying length cannot be satisfied out of the bytes of the components behind it. A built-in frame is unframed and
  has no such bound, so it still reads through the stream itself.
