...
public sealed class SqlServerJournalResetContentionTests
{
    ...
    public async Task A_reset_waits_behind_the_lock_and_goes_through_once_it_is_released()
    {
        ...
        Assert.True(await TakeAsync(holder, held, "Shared") >= 0, "The holder must get the lock before the reset runs.");
    ...
    private static async Task AssertRefusedWhileHeldAsync(string mode)
    {
        ...
        await using (var holder = new SqlConnection(SqlServerJournalResetHarness.ConnectionString))
        {
            await holder.OpenAsync();
            await using SqlTransaction held = (SqlTransaction)await holder.BeginTransactionAsync();
            Assert.True(await TakeAsync(holder, held, mode) >= 0, "The holder must get the lock before the reset runs.");
    ...
    /// <summary>
    /// The maintenance lock on the resource name the store names, in the mode a commit or a maintenance call takes,
    /// with no wait at all so a failure to get it is reported here rather than hanging the fact.
    /// </summary>
    private static async Task<int> TakeAsync(SqlConnection connection, SqlTransaction transaction, string mode)
    {
        await using SqlCommand command = connection.CreateCommand();
