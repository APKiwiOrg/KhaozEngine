...
namespace KhaozEngine.Showcase
{
    ...
    /// runtime-baked TTF text, batched through <see cref="SpriteBatch"/>. The whole composition is drawn in the
    /// design-space pass so the header bar, sprites, and captions scale and centre together as one unit on resize
    /// (rather than the captions reflowing independently of the sprites); its fonts are oversample-3, which the
    /// design-to-framebuffer scale minifies into crisp text. The room owns no GPU device itself (a
    /// <see cref="GameScene"/> cannot reach one) - <see cref="ShowcaseApp"/> creates the texture/fonts on its
    /// <c>Surface2D</c> and hands them in via <see cref="Init"/> right after construction, keeping the constructor
    /// itself parameterless for the room registry's <c>Func&lt;GameScene&gt;</c> factory.</summary>
    public sealed class Room2D : GameScene
    {
        ...
        // Header bar + checker sprites + captions, all in design space so they stay aligned and centre together as
        // the window resizes. The captions are developer-facing chrome, not localizable player copy, so the raw
        // DrawString literals are intentional (the KELOC003 escape hatch).
        [LocalizationExempt]
