...
/// connection behind it. <see cref="TileMovementSystem"/> reads this tag to pick which
/// <see cref="TileMoveSimulator"/> steps the entity, which is how an actor paths at a leash-sized radius while a
/// player paths at a click-sized one.
...
/// an entity from its Migrate capture, so a migrated actor arrives WITHOUT this tag, exactly as it arrives without
/// its <see cref="PendingTileCommand"/>. <c>TileActorHost</c> re-adds both every tick for every live actor, which is
