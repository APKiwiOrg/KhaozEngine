...
/// configuration the game hands the engine (<c>TileStepTicks</c>), never a constant in here. A tile world's feel
...
{
    /// <summary>The slower rate, one tile per <c>TileStepTicks.Walk</c> ticks. The default a fresh state starts in,
    /// so a player who never touches the run toggle walks.</summary>
    Walk = 0,

    /// <summary>The faster rate, one tile per <c>TileStepTicks.Run</c> ticks. Nothing here caps or meters it: a run
    /// energy budget is game rules, and belongs above this package.</summary>
    Run = 1,
}
