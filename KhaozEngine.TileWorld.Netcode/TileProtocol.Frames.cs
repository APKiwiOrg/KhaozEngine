...
/// other, and the notice frame carries the same rule so no envelope on this wire is an exception to it. The
/// snapshot frame is deliberately NOT padded: it is variable length by nature, travels server-to-client only, and
/// nothing in that direction is a fixed-size frame it could be confused with.</para>
/// <para>Every decoder here is total, on the same grounds as the command decoder: it returns false for a
/// truncated, mis-tagged, over-capped or internally inconsistent frame and never throws, because the bytes come
/// from a remote peer.</para>
/// </summary>
public static partial class TileProtocol
{
    ...
    // [tag:1][localNetId:8][ackSeq:4][serverTick:8], then the replication snapshot verbatim.
    const int SnapshotHeader = 1 + 8 + 4 + 8;
    ...
    /// snapshot was taken on (so a discrete tile sample lands on the right point of the delayed render timeline).
    /// </summary>
    /// <exception cref="ArgumentNullException"><paramref name="snapshot"/> is null.</exception>
    public static byte[] EncodeSnapshotFrame(long localNetId, int ackSeq, long serverTick, byte[] snapshot)
    {
        ArgumentNullException.ThrowIfNull(snapshot);
        var b = new byte[SnapshotHeader + snapshot.Length];
    ...
    /// the frame is shorter than the header or carries another tag. The snapshot is COPIED out rather than sliced,
    /// because <c>ClientReplicationView.Apply</c> takes an array and the copy is one allocation per snapshot on a
    /// path that already allocated the frame.</summary>
        ...
        out long serverTick, out byte[] snapshot)
    {
        ...
        if (data.Length < SnapshotHeader || data[0] != ServerFrameSnapshot) return false;
        ...
        snapshot = data.Slice(SnapshotHeader).ToArray();
        return true;
    }
    ...
    /// <see cref="MaxGameMessageBytes"/>.</exception>
