...
| **KhaozEngine.Render2D** | `SpriteBatch` (textured quads + `DrawString` over stb_truetype `SpriteFont`), `Camera2D`, `Texture2D`, `Render2DSurface`, scissor/point-sampling, offscreen capture. | Windowing, Gpu |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="5.63.1" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="5.63.1" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="5.63.1" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="5.63.1" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
