...
| **KhaozEngine.Physics.Bepu** | BepuPhysics v2 backend (opt-in, NOT in any umbrella; add explicitly like `WorldStore.Sqlite`): `BepuPhysicsWorld : IPhysicsWorld` over BepuPhysics 2.4.0 (pure-managed, Apache-2.0). Single-threaded deterministic `Simulation` (4 substeps, uniform gravity; default Earth gravity, `new BepuPhysicsWorld(Vector3.Zero)` for static-only). `AddStatic`/`RemoveStatic`, `AddDynamic`/`RemoveDynamic` (dynamic rigid bodies with shape-derived inertia and explicit deterministic restitution), `Step(dt)`, raycast, sweep-capsule, penetration. Wire into `CharacterController3D` / `WorldServer` / `WorldClient` via the `IPhysicsWorld?` ctor param. | KhaozEngine.Physics, BepuPhysics |
