...
| **KhaozEngine.Gui** | `GuiSurface` (immediate-mode UI: Panel/Label/Button/Slider/Toggle, hover, click-through gate), `ScreenStack` (top-to-bottom routed screen stack + widgets), `FocusNavigator` (menu navigation; `Toggle`/`Slider`/`Dropdown` also take an opt-in `Update(InputManager, focused)` for keyboard/gamepad control), `DiagnosticsOverlay` (F1-toggled corner telemetry HUD: game-built `OverlaySection`/`OverlayRow`s + `PerformanceSection`/`NetworkSection` populators, headless-testable `Update`). Player-facing text sinks (Label/Button widgets, `GuiSurface.Label`/`Button`/`StatChip`, `Tooltip.Show`) take a `LocalizedText` (from `KhaozEngine.App`) instead of a raw `string`; the old `string` overloads remain `[Obsolete]`. `IconButton`'s string is an icon-atlas key, not localized. | App, Windowing, Render2D, Updates, Diagnostics |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="10.9.0" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="10.9.0" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="10.9.0" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="10.9.0" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
