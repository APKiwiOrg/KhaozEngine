...
| **KhaozEngine.Replication** | Authoritative ECS replication: `NetId` + a closure-based `ReplicationRegistry` (consumer extension component ids >= `FirstExtensionTypeId` are length-prefixed on the wire, so an older client skips an id it never registered instead of disconnecting; optional per-registration `ReplicationChannels` - `Replicate`/`Persist`/`Migrate`/`OwnerOnly` - so a component can be server-only or owner-only rather than always persisted==replicated==migrated), `SnapshotWriter` (full-state + `WriteFiltered` per-client interest), `ClientReplicationView` (`Apply`/`ApplyDelta`: spawn/despawn/update + interpolation, skip-unknown-extension, self-healing acked baseline), `ServerReplicator` (per-slot acked whole-world baseline/delta), `AoiDeltaReplicator` (per-client, `NetId`-keyed, area-of-interest-scoped baseline/delta - entered->full, changed->component delta, left->despawn; what the servers serve on the live path), and `InterestGrid` (area-of-interest spatial query). Transport-free (snapshots/deltas are `byte[]`). | Ecs |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="9.32.1" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="9.32.1" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="9.32.1" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="9.32.1" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
