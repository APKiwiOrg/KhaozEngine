...
| **KhaozEngine.Locomotion** | Render-free character locomotion core: `CharacterMovement.Step` from a `MoveCommand` (camera-relative WASD axis + run + camera yaw + jump) over a timestep, normalized diagonals, ground-clamped via a height delegate with an optional slope gate. Two overloads share one horizontal core: `Step(Vector3,...)` (horizontal-only, Y instant-clamped) and the vertical-physics `Step(in MoveState,...)` (gravity + jump with coyote-time/jump-buffer + air control over a carried `MoveState`, plus optional 3D prop collision via `IPhysicsWorld?`). One `MoveTuning` source of truth (speeds + gravity/jump/fall/feel) shared by the local `CharacterController3D`, the authoritative server sim, and client prediction. No input/render/netcode. In the `Foundation` umbrella. | Primitives, Physics |
...
| **KhaozEngine.PropSurface.Tool** | The `ke-propbake` dotnet tool (`PackAsTool`): bakes a walkable-surface `.surf` heightmap and a 3D collision shape (`CollisionShape`) per walkable-solid prop (rocks/logs/buildings) in a kit manifest, stamping the `surface`/`heightmap` fields and feeding `PropCollisionLoader` for physics wiring. Run as the last kit-ingest step (re-ingest = re-bake). Author-time tool, not a runtime package. | Render3D |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="8.1.0" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="8.1.0" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="8.1.0" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="8.1.0" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
