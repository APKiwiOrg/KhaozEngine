...
| **KhaozEngine.Render3D** | Stylized 3D: `Scene3D` (multi-instance mesh draw, glTF + procedural meshes, per-mesh albedo textures, materials/lighting, billboards, runtime GPU bone-palette skinning for code-driven deformation, a splat-material pipeline for PBR terrain (`LoadSplatMaterial`/`SplatMaterialHandle`), debug draw), `IsoCamera3D` (ortho iso + screen-to-ground/ray picking), world-space overlays (`WorldLabel` text + the `NameplateRenderer` name/bars plate), `PixelPostProcess`, `Render3DSurface`. Shadows (`ShadowMode` Off/Blob/ShadowMap via `RenderQuality.Shadows`, `Scene3D.AddShadowBlob`), always-on frustum culling (`DrawnInstances`/`CulledInstances` counters), per-pass CPU timing (`Scene3D.EnableTiming`/`PassTimingsMs`, off by default), a gradient sky (`PixelPostProcessSettings.Sky`, key-light-aligned sun), an opt-in LDR threshold + separable-blur bloom (`PixelPostProcessSettings.Bloom`, default off), and an opt-in animated water surface (`Scene3D.DrawWater`/`PixelPostProcessSettings.Water`: normal perturbation, sky-derived fresnel tint, sun glint, depth-sampled shore fade, default off/no-op). | Ecs, Windowing, Gpu, Render2D, Collision, Physics, Primitives |
| **KhaozEngine.Gui** | `GuiSurface` (immediate-mode UI: Panel/Label/Button/Slider/Toggle, hover, click-through gate), retained widgets (`Button`/`Slider`/`Toggle`/`Dropdown`/`TextInput`/`TabBar` segmented control), `ScreenStack` (top-to-bottom routed screen stack + widgets; exposes a shared `InputManager` so screens drive keyboard/gamepad nav), `FocusNavigator` (menu navigation; `Toggle`/`Slider`/`Dropdown` also take an opt-in `Update(InputManager, focused)` for keyboard/gamepad control), `DiagnosticsOverlay` (F1-toggled corner telemetry HUD: game-built `OverlaySection`/`OverlayRow`s + `PerformanceSection`/`PassTimingsSection`/`NetworkSection` populators, headless-testable `Update`), `GuiTheme` (the crisp neutral-dark default palette all widgets read; settable `GuiTheme.Default` for a one-line global reskin, `GuiTheme.Legacy` to keep the pre-10.11.0 look) + `GuiStyle` semantic presets (`Primary`/`Secondary`/`Danger`/`Active`, `Modern`, `Legacy`). Player-facing text sinks (Label/Button widgets, `GuiSurface.Label`/`Button`/`StatChip`, `Tooltip.Show`) take a `LocalizedText` (from `KhaozEngine.App`) instead of a raw `string`; the old `string` overloads remain `[Obsolete]`. `IconButton`'s string is an icon-atlas key, not localized. | App, Windowing, Render2D, Updates, Diagnostics |
...
| **KhaozEngine.Updates** | Delta auto-update pipeline: SHA256 manifests + diffing, a host-agnostic update source, an `UpdateService` state machine with resumable staged downloads, a cross-platform staged-apply core (`UpdateApplier`) with an atomic file swap and a resilient post-apply relaunch (settle wait + antivirus/image-race retry), and an optional native Win32 progress window (`IUpdaterUi`) for the shim. | Diagnostics |
...
| **KhaozEngine.Foundation** | the GPU-free foundation (Primitives/App/Content/Diagnostics/Ecs/Identity/Locomotion/Objectives/Persistence/Progression/Serialization/Social/Collision/Physics/Terrain/Determinism/Platform/Updates) | a gameplay-logic library (no renderer) |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="10.28.0" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="10.28.0" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="10.28.0" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="10.28.0" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
...
KhaozEngine.Terrain/   KhaozEngine.Updates/   KhaozEngine.Progression/
