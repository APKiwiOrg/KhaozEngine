...
| **KhaozEngine.Imaging** | Dependency-free image IO: a minimal built-in PNG encoder (`PngWriter`) for 8-bit RGBA buffers (BCL `ZLibStream` + a CRC-32 table only), so no third-party image library enters the graph. The canonical home of the encoder the snapshot harness and Render2D's clipboard-copy path use. Encode-only (no decode/palette/interlace). | Pure .NET |
...
<PackageReference Include="KhaozEngine.Game2D"     Version="10.16.0" />  <!-- desktop 2D: 2D runtime + GameApp/SceneManager + foundation -->
<PackageReference Include="KhaozEngine.Game3D"     Version="10.16.0" />  <!-- desktop 3D: Game2D + Render3D + the 3D scene bridge -->
<PackageReference Include="KhaozEngine.Server"     Version="10.16.0" />  <!-- headless: foundation + netcode, no graphics -->
<PackageReference Include="KhaozEngine.Foundation" Version="10.16.0" />  <!-- gameplay-logic lib: foundation only, no renderer/netcode -->
```
