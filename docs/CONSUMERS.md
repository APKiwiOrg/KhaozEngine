...
**Engine current version:** `7.14.1` (the shared `<KhaozEngine5xVersion>` line, which is the engine): the
...
`KhaozEngine.Updates.Tool` package (the `ke-updater` dotnet tool: manifest/genkey/sign/verify), and 7.14.1 adds
...
### Nullwake - 7.x, 2D (on `KhaozEngine.Game2D` 7.12.0)
...
_Last verified: 2026-06-21. The shared line `<KhaozEngine5xVersion>` = **7.12.0** is the engine: the new
