...
**Engine current version:** `9.12.0` (the shared `<KhaozEngineVersion>` line, which *is* the engine). The
...
| **Nullwake** (2D) | `Nullwake.Core` | `KhaozEngine.Game2D` + `Diagnostics`/`Persistence`/`Windowing` + `Updates` (shim, dormant) + `Snapshot` (dev tool); uses `AttentionBeacon`, clipboard paste + `Pointer.WindowFocused` gating in name entry, `GameAppOptions.WindowIcons` + `WindowIconPath` (multi-size taskbar icon + macOS Dock icon, 9.8.0), and `GameApp.OnResume` + `GameClock.RealWallGapSeconds` (9.12.0, resume-from-sleep offline catch-up). Vendored feed (`Nullwake/vendor/khaozengine`) | **9.12.0** |
