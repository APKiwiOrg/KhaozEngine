...
**Engine current version:** `7.9.0` (the shared `<KhaozEngine5xVersion>` line, which is the engine): the
...
carried to 7.9.0 for the hover-glow fix), **Nullwake** (2D) is on **7.6.0** via `Game2D` (+ `Diagnostics`/
