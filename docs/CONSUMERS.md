...
**Engine current version:** `8.8.0` (the shared `<KhaozEngineVersion>` line, which *is* the engine). The
...
_Last verified: 2026-06-30. **Ruinborne** (3D MMO, the active consumer) now pins **8.5.2** (the current engine
line); **Hardpoint**, **Nullwake**, and **SpaceGame** remain on **8.0.0** (pre the 8.4.0 swept character resolver,
so the 8.5.2 float-up/wall-hang fix does not affect them; only Ruinborne runs the 3D `CharacterMovement` swept
collide-and-slide). Stack per consumer: **Hardpoint** (3D) via
