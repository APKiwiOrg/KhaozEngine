...
The durable backend adds these edges, and nothing references it back. It is on the `OptInBackends` list too, and
`ThirdPartyHomes` names it as a home of the driver:
...
| Account registry | `../KhaozEngine.Accounts/IAccountStore.cs`, `AccountRecord.cs`, `AccountSignIn.cs`, `AccountStoreRules.cs` | `../KhaozEngine.Accounts/InMemoryAccountStore.cs`, `AccountBanStore.cs`, `../KhaozEngine.Accounts.Sqlite/SqliteAccountStore.cs`, conformance in `../KhaozEngine.Accounts.Tests/AccountStoreConformance*.cs` |
