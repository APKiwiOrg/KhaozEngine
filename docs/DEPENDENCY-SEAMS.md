...
- `ArchitectureTests.cs` - third-party containment (every third-party PackageReference stays in its allowlisted
  ...
  every umbrella's transitive closure, the `Microsoft.AspNetCore.App` framework reference staying in its two web
  packages (`AspNetCoreFramework_IsContainedToWebPackages`), the INVERSE rule for the three native GPU backends (every umbrella that
...
| Account registry | `KhaozEngine.Accounts` (`IAccountStore`: find-or-create from a verified `AccountSignIn`, read, keyset listing in ordinal order, the filed-ban list, and the whitelist and ban writes, none of which creates a row. The `AccountRecord`, `AccountBan` and `AccountSignIn` values, and `AccountStoreRules`, the subject minting, reserved `guest:` prefix and length rules every engine store applies alike. The STORE mints the subject, so a game adapter over its own schema can mint a provider-neutral id) | `InMemoryAccountStore` ships in the seam package as the reference and test store. `AccountBanStore` in the same package is the ONE `IBanStore` over any account store (see the ban row below). The two durable backends are OPT-IN siblings in no umbrella: `KhaozEngine.Accounts.Sqlite` (`SqliteAccountStore`, over `KhaozEngine.Sqlite`'s `SqliteStoreConnection` like every other SQLite store in the engine) and `KhaozEngine.Accounts.SqlServer` (`SqlServerAccountStore`, a fresh pooled connection per call and a lazy first-call bootstrap under `AutoCreate` or `ValidateOnly`), both on Grimhollow's flat `accounts` layout under a configurable table name so that database is adopted in place. The SQL Server leg of the conformance suite is gated on `KE_ACCOUNTS_SQLSERVER`, which CI's `accounts-sqlserver` job sets against a SQL Server service. A game adapter over its own schema implements the seam directly. The shared `AccountStoreConformance` suite in `KhaozEngine.Accounts.Tests` is the contract every engine backend runs, one subclass each | Microsoft.Data.Sqlite / SqlClient, only in the two provider packages. The seam package itself takes no third-party dependency and no SQL |
...
| Server ban list | ONE seam, `IBanStore` (+ `BanRecord`, `InMemoryBanStore`), consulted live on the host thread. It lives in the `KhaozEngine.Netcode` ASSEMBLY under its shipped `KhaozEngine.NetWorld` names, and `NetWorld` type-forwards all three, so a consumer naming `KhaozEngine.NetWorld.IBanStore` compiles and binds unchanged while `TileWorld.Netcode` takes it without referencing `NetWorld`. The DOOR is `KhaozEngine.Netcode.BanGateAuthenticator(inner, IBanStore)`, which refuses a banned account with `HandshakeToken.BannedReason` before a player entity exists at all. Its `Func<string, bool>` constructor stays for a head whose ban list is not a store | `WorldStoreBanStore` (persists over any `IWorldStore` keyspace `ban:{accountId}`, stays in `NetWorld` because it needs `WorldStore`). A float head hands ONE store to the gate and to `banStore:`, where `WorldServer` and `ShardedWorldServer` also check it at JOIN. A tile head hands the same store to `TileWorldServerConfig.BanStore`, read at the door, at the join and once per tick over every live session, which it closes with the `ke:banned` notice token. `KhaozEngine.Accounts.AccountBanStore` adapts the account store to this seam, so the account row is the only place a ban is filed: a game on that seam hands the one `AccountBanStore` to the door, `banStore:`, `TileWorldServerConfig.BanStore` and `ServerAdmin(bans:)`, and must not also wire `WorldStoreBanStore`, whose `ban:{accountId}` keys would be a second list | (no extra dep, sync `IsBanned` via in-memory cache, `LoadAsync()` at startup) |
| Player persistence, the record-agnostic core | `KhaozEngine.WorldStore` (`IPersistenceHost<TState>`, the head's side: `PlayerJoined` / `PlayerLeaving` / `SetPlayerState` / `JoinedSlots` / `TryGetAccountId` / `TrySetPersistenceKeyResolver` / `TryGetPersistenceKey` / `TryGetPlayerState`, plus the join-seed pair `SetPositionHintProvider` + `TryGetConfiguredSpawn` as default interface methods. Plus `PersistenceBinding<TState>` - `PositionOf` / `Encode` / `Decode` (`RecordDecoder<TState>`) / `Validate` - which is the whole of what a movement model contributes) | `StatePersistence<TState>` (+ `PersistenceCoreConfig` and `PositionHintCache` / `PositionHintProvider` behind the join seed) installs the optional pre-spawn resolver, then owns the interval, the dirty pass, the per-session load guard, the per-key write ordering, quarantine, the guest policy and the rejoin hints for every head in the fleet | (no extra dep. `KhaozEngine.WorldStore` stays zero-dependency: the core's own log lines leave through `PersistenceCoreConfig.Diagnostic`, an `Action<string, Exception?>` a head wires to its own logging) |
| Player persistence, the float binding | `KhaozEngine.NetWorld` (`IWorldPersistenceHost`, the surface `WorldPersistence` drives, implemented by BOTH `WorldServer` and `ShardedWorldServer`, which since 17.40.0 DERIVES from `IPersistenceHost<PlayerMoveState>` and inherits every member below verbatim rather than declaring it: `PlayerJoined` / `PlayerLeaving` / `SetPlayerState` / `JoinedSlots` / `TryGetAccountId` / `TrySetPersistenceKeyResolver` / `TryGetPersistenceKey` / `TryGetPlayerState`, and since 17.37.0 the join-seed pair `SetResumePositionProvider` + `TryGetConfiguredSpawn`, both default interface methods so an existing implementer keeps compiling and simply keeps spawning every join at its configured spawn. `SetResumePositionProvider` keeps its NetWorld name and is bridged onto the generic `SetPositionHintProvider` by a re-implementation on this interface, so no implementer ever sees the generic one) | `WorldPersistence` (+ `WorldPersistenceConfig`, `PlayerRecord`, and the `ResumePositionCache` behind the join seed) is the FLOAT binding of `StatePersistence<PlayerMoveState>` (the row above) and wires it to any `IWorldStore`, keyed `player:{persistenceKey}` (the verified account subject by default, or one immutable pre-spawn resolver result). A TOKENLESS connection, which both heads hand over as `guest:{slot}`, is not persisted at all unless `WorldPersistenceConfig.PersistGuests` is set, and then under a durable per-session key rather than the seat) | (no extra dep, it rides whichever `IWorldStore` backend the game chose) |
| Tile-world movement | `KhaozEngine.TileWorld.Netcode` (`TileWorldServer` / `TileWorldClient` over `Netcode`, `Replication`, `Sharding`, `Simulation`, `WorldStore` and `TileWorld`) | a SIBLING of `NetWorld`, not a dependent: the two movement stacks share the generic layers and nothing else, so a tile server never carries the float locomotion stack. `TileMoveSimulator` is the discrete `ITickSimulator` both heads run. `TileActorTraversalProfile` is an opaque game key that binds a server-owned actor to a registered collision map while players and prediction keep the constructor map. `TileCollisionBaker.Bake` can replace only the ground rule for such a map, then applies its ordinary object and wall passes. `TileWorldPersistence` is the tile binding of the persistence core two rows up, and the connect door is `Netcode.ConnectionGate` rather than anything tile-shaped. The admin seam `IAdminControllable` and the ban seam `IBanStore` both live in the `Netcode` assembly under their `NetWorld` names, so `TileWorldServer` implements the one and takes the other without the edge. Enforced by `ArchitectureTests.TileWorldNetcode_NeverReferencesNetWorld`, which asserts it on the package AND on its test project | (no extra dep) |
| Tile target resolution | `KhaozEngine.TileWorld.Netcode` (`ITileTargets`, one required footprint-and-plane method plus default `TryGetAimPoint` and `GetInteractionReachPolicy` methods. `TileInteractionReachPolicy` can add diagonal and valid-overlap anchors through the shared `TileInteractionReach` kernel) | THREE implementations across TWO id spaces, which is the point of the seam. `TileDocumentTargets` is the read-through OBJECT space. `TileEntityTargets` is the server's tick-start ENTITY snapshot, excluding ghosts and holding a migrating entity only for its bounded grace window. `TileRemoteTargets` is the client's newest entity read. The optional seventh `TileWorldServer` and `TileWorldClient` constructor argument wraps those engine-owned entity resolvers with the same per-target policy selector on both heads. **TWO SPACES rather than one resolver is mandatory:** authored ids and net ids overlap exactly, so `TileCommandKind` discriminates them and `TileMoveSimulator` carries both resolvers. The entity resolver remains the final constructor argument on `TileMoveSimulator`, preserving positional callers. Combat continues to use default `TileReach` and ignores interaction policy | (no extra dep) |
| Tile ground height | `KhaozEngine.TileWorld.Netcode` (`ITileGroundHeight`, one `HeightAt(float tileX, float tileZ, int plane)` taking TILE units on the lattice and answering world METRES, which `TilePresenter` samples at the planar centre it draws the pose at) | `TileDocumentGroundHeight` is the engine's only implementation, the document's own bilinear lattice, and it is the ONE place tile units become world metres for a height read, which keeps the tile-z negation inside `TilePresenter.cs` and that one adapter. `TilePresenter(TileWorldDocument)` wires it, so a head built from the world file gets terrain with no call of its own. NULL is a first-class answer rather than a missing implementation: the `(tileSize, planeHeight)` presenter has no source and draws every pose at its plane index times `PlaneHeight`, which is what `TileWorldClient` installs before a document is loaded, and `TilePresenter.Ground` is how a head tells the two apart. A game substitutes its own for a streamed or generated terrain | (no extra dep) |
| Tile actor behaviour | `KhaozEngine.TileWorld.Netcode` (`ITileActorBehaviour`, one `Decide` over a `TileActorContext` returning a `TileActorIntent`, plus `TileActorRandom`, the per-actor splitmix64 stream that lets an implementation stay stateless) | `TileWanderBehaviour` is the engine's own default (leash, chase, retaliate, stand-your-ground, wander, in that order), and a game replaces it wholesale rather than extending it. Nothing installs it: `TileActorHost.Behaviour` is null by default, so an actor with no behaviour stands where it was put. ONE behaviour instance is shared by every actor. The seam's line is drawn at exactly one place: an intent names a TILE or a TARGET and never a route, a step, a facing or a tick, so everything about HOW an actor moves stays inside the stepper both heads run and an actor can never move by a rule a player could not. The context carries a RESOLUTION answer for the held target and both damage records (`TargetResolved`, `LastDamagedByResolved`, `LastAttackedByResolved`), plus the actor's registered `TraversalProfile` and `TraversalMap`. The shipped wander behaviour uses that map for destinations, so mixed topologies do not leak through the behaviour seam | (no extra dep) |
| Tile actor spawn admission | `KhaozEngine.TileWorld.Netcode` (`TileActorHost.SpawnAdmission`, one synchronous predicate over the ready `TileActorSpawner`) | **game-side**, null by default so every ready authored spawner is admitted. A false answer keeps an initial spawner `Empty`, or a due respawner `Waiting` at zero ticks, and retries on a later authoritative tick before actor allocation, linkage, or the spawn event. The consumer keeps durable and remote work outside the tick, then exposes its completed state through an in-memory lookup. Direct `TileWorldServer.SpawnActor` calls bypass the seam | (no extra dep) |
| Tile combat rules | `KhaozEngine.TileWorld.Netcode` (`ITileCombatRules`: `Roll` over a `TileAttackContext` returning a `TileAttackOutcome`, and `AttackTicks` per attacker). The context carries both net ids, both committed tiles, both committed footprints (`AttackerFootprint` and `TargetFootprint`, trailing and defaulted since 19.0.0, for a rule that measures geometry itself), both healths and the tick | **game-side**, no engine implementation and deliberately none. The line is where a second game would disagree: the engine owns whether a swing is DUE (the cooldown) and whether it is LEGAL (adjacency through `TileReach`), and this owns what it DOES. An actor's legality check uses its registered traversal map, which keeps final reach geometry aligned with its chase without adding combat styles or ranged rules. Players keep the constructor map. Null means nothing ever swings, which is the right default for a head that has not wired combat. The roll is NEVER predicted by a client, so the seam needs no cross-head determinism at all, only server-side reproducibility, which it gets from the engine's fixed roll order (oldest lock first, net id breaking the tie) | (no extra dep) |
| Tile object look override | `KhaozEngine.TileWorld.Render3D` (`TileObjectProps.Build`'s optional `Func<long, string?> archetypeOverride`, and `TileWorldView.OverrideArchetype` / `ClearOverride` over it) | **game-side**, a function rather than an interface because it answers one question with one argument: which archetype does object N draw as. Null, or a null answer, draws the document's own, so a head that never sets one pays nothing and the seam adds no state to a view that does not use it. It is what turns `TileWorldClient.ObjectStateChanged` into a picture, and it is deliberately not state-shaped: the engine hands over an object id, never a `TileObjectState`, so a look that has nothing to do with replication (a season, a time of day, an editor preview) rides the same seam | (no extra dep) |
...
| Content pack store | `KhaozEngine.Catalog` (`IPackStore`: `ExistsAsync` / `GetAsync` / `PutAsync` / `ListAsync`, four members over a CONTENT-ADDRESSED store, with the delete path split out into `IPackStorePruning` so a read-only provider cannot be asked to prune and a misconfigured one cannot delete a production pack through the common interface. `IContentVersionPointerSource` is a separate read-only seam for `versions/<n>`, the one object in the store not named by its own hash, kept off `IPackStore` so a read-only provider cannot be a half-working publish target) | flavour 2 below for THREE of them, all in the seam package because none takes a dependency. `FileSystemPackStore` is the local one and the only one that also implements `IContentVersionPointerSource` and `IPackStorePruning`. It samples an open file's length against `ContentPackFormat.MaxObjectBytes` before allocating its contents, and it reads one probe byte past the sample so concurrent growth is still bounded. `HttpPackStore` is read-only over `HttpClient` and applies the same ceiling to declared and chunked bodies. `CachingPackStore` is the decorator a client runs: local in front, remote behind, every fetched body verified against its content address before it is kept and a refusal reported through its `onRefused` callback. The FOURTH is the opt-in sibling that declares its SDK there rather than here: `KhaozEngine.Catalog.AzureBlob`'s `AzureBlobPackStore`, `IPackStore` plus `IPackStorePruning` over one blob container, holding hash objects and NEITHER pointer interface, so a publish or a rebuild that resolves the pointer half refuses it at run time with `no-pack-store` and a full server pack can never reach a public container. `FileSystemPackStore.RelativeKeyFor` is the ONE statement of the key layout (`<hash[0..2]>/<hash[2..4]>/<hash>.kec`, lower case, forward slashes, no leading slash) and all three providers that derive a name from a hash go through it, which is what lets `HttpPackStore` read what the blob store wrote with no client change. `ContentOriginFill` is the copier over the seam: `ContentFetchLoop` with the origin as its local half, so the closure is walked once in the engine rather than once per direction | (no extra dep, `System.Net.Http` is in box and only `HttpPackStore` touches it. `Azure.Storage.Blobs` is declared in the opt-in blob package alone) |
| Content authoring store | `KhaozEngine.Catalog.Authoring` (`IContentAuthoringStore`, TWENTY-NINE members and the one shape every backend implements: schema init and version, the store epoch, the active version and the operator pin, the one open draft with its edits, freeze and discard, the publish baseline, commit and publish, rollback, row page and history reads, audit reads and the operational audit append, id allocation plain and in-family, family listing and creation, bundle import and export, plus the `PackStore` the operational pair works on. It INHERITS `KhaozEngine.Catalog`'s `IContentVersionDirectory`, which is where the active version and the operator pin are declared, so the boot's version seam and this one have ONE declaration of each and a host that boots off its authoring database hands `ContentBootOptions.Directory` the store itself. Pinned member by member by a test, over the seam's own declarations plus its inherited interface's, so a member added to one provider and not the others is red rather than latent. `IContentIdPersistence` is the smaller seam `ContentIdAllocator` sits on, and `IContentRowSideEncoder` is the publish's row-encoding seam) | `InMemoryContentAuthoringStore` ships in the seam package for tests and tools, and the two durable backends are OPT-IN siblings in no umbrella: `KhaozEngine.Catalog.Sqlite` (`SqliteContentAuthoringStore`, over `KhaozEngine.Sqlite`'s `SqliteStoreConnection` like every other SQLite store in the engine) and `KhaozEngine.Catalog.SqlServer` (`SqlServerContentAuthoringStore`, a fresh pooled connection per call, every write `Serializable`). The same conformance suite runs against all three, and the SQL Server leg is gated on `KE_CATALOG_SQLSERVER`, which the `catalog-sqlserver` CI job sets against a real instance | Microsoft.Data.Sqlite / SqlClient, only in the two provider packages. The seam package itself takes NO third-party dependency and no SQL |
...
`KhaozEngine.Server.Admin` sits outside the seam/backend pattern: it is not a backend (the server heads implement
`IAdminControllable` themselves), but it is deliberately NOT in the `Server`
umbrella because it references ASP.NET Core (via a `FrameworkReference` on `Microsoft.AspNetCore.App`). It is one of
the two packages that do, with the sign-in exchange handler `KhaozEngine.Identity.Exchange.AspNetCore` (see the auth
exchange edge below), and `ArchitectureTests.AspNetCoreFramework_IsContainedToWebPackages` fails the build if a third
packable project takes the framework reference. Its dependency edge is:

```
KhaozEngine.Server.Admin -> KhaozEngine.NetWorld (ServerAdmin, plus IAdminControllable and IBanStore through NetWorld's Netcode edge)
...
`IAdminControllable` and the three types it names (`PlayerRef`, `OnlinePlayer`, `MovementCommitmentRequest`) live
in the `KhaozEngine.Netcode` ASSEMBLY under their shipped `KhaozEngine.NetWorld` names, and `NetWorld` type-forwards
all four (`TypeForwards.cs`), which is the same move the ban seam made. That is what lets
`KhaozEngine.TileWorld.Netcode`'s `TileWorldServer` implement the interface with no `NetWorld` reference, so a tile
head hands itself to `ServerAdmin` exactly as a `WorldServer` or `ShardedWorldServer` does. `ServerAdmin` itself
stays in `NetWorld`, so a tile game that wants the facade or the endpoint takes `NetWorld` in its own head, which is
the head's choice and never the tile package's. `MovementCommitmentRequest` used to build a Locomotion
`MovementCommitment` to validate itself, and `Netcode` sits below `Locomotion`, so the request now restates those
refusals and `MovementCommitmentRequestParityTests` in `KhaozEngine.Server.Tests`, the one project that sees both,
holds the copy equal. `MovementCommitmentResult` names a Locomotion enum and stays in `NetWorld`.
...
**The ASP.NET Core handler.** `KhaozEngine.Identity.Exchange.AspNetCore` mounts the core as `POST /auth/exchange` and
configures the host around it. Its edges are:

```
KhaozEngine.Identity.Exchange.AspNetCore -> KhaozEngine.Identity.Exchange   (AuthExchange, AuthExchangeResult and the Identity wire DTOs it serializes)
KhaozEngine.Identity.Exchange.AspNetCore -> Microsoft.AspNetCore.App [shared framework, FrameworkReference]
```

The rate limiting (`System.Threading.RateLimiting`), the forwarded headers and the routing metadata it uses are all in
that shared framework, so it adds no package. It is the second package after `Server.Admin` to take the framework, and
it stays out of every umbrella on the same precedent: a sim server that runs no auth endpoint must not carry the web
stack. `ArchitectureTests.IdentityExchangeAspNetCore_ReferencesOnlyTheExchange` pins its edge,
`ArchitectureTests.AspNetCoreFramework_IsContainedToWebPackages` holds the framework reference to these two packages
among packable projects, and `Identity.Exchange.AspNetCore` is on the `OptInBackends` list.
...
`KhaozEngine.Accounts` is a seam package in the sense above: engine edges only, no third-party package, and
headless-testable against its in-memory store. Its one edge is:

```
KhaozEngine.Accounts -> KhaozEngine.Netcode   (IBanStore and BanRecord, which AccountBanStore implements and returns)
```

It is a package of its own, rather than a section of whatever runs the sign-in exchange, because a game server and
an admin console need the store and the ban adapter without an exchange or `KhaozEngine.Identity` in their graph.
It is in NO umbrella, on the `Commerce` precedent rather than the `WorldStore` one: a server-only seam stays opt-in
until more than its first consumers want it. `ArchitectureTests.Accounts_ReferencesOnlyNetcode` pins the edge and
the absence of any package reference, and `Accounts` is on the `OptInBackends` list, so no umbrella can reach it.

The two durable backends add these edges, and nothing references either back. Both are on the `OptInBackends` list
too, and `ThirdPartyHomes` names each as a home of its driver:

```
KhaozEngine.Accounts.Sqlite    -> KhaozEngine.Accounts      (the seam it implements)
KhaozEngine.Accounts.Sqlite    -> KhaozEngine.Sqlite        (SqliteStoreConnection, the shared unpooled open/gate/dispose lifecycle)
KhaozEngine.Accounts.Sqlite    -> Microsoft.Data.Sqlite     (the driver, declared HERE and nowhere below it)
KhaozEngine.Accounts.SqlServer -> KhaozEngine.Accounts      (the same seam)
KhaozEngine.Accounts.SqlServer -> Microsoft.Data.SqlClient  (the driver, declared HERE and nowhere below it)
```
...
**The engine subject rule.** The engine stores mint `{ProviderId}:{ProviderSubject}` through
`AccountStoreRules.MintSubject`, refuse a `.` anywhere (a `SignedToken` splits on it) and the reserved `guest:` prefix
(it names a tokenless seat, which persistence and the join gate both refuse), and bound the subject, display name
and ban reason at 128, 128 and 256 UTF-16 code units. The prefix literal restates
`PositionHintCache.GuestAccountPrefix`, which `Accounts` cannot reference, and a parity fact in
`KhaozEngine.Accounts.Tests` holds the two equal.
...
support project retains direct backend edges for its uniform-ring adapters and its filtered registration-test
module initializer belt.
...
KhaozEngine.TileWorld.Editing -> KhaozEngine.Imaging    (PngReader, full-precision PNG height imports)
...
KhaozEngine.TileEdit.Tool -> KhaozEngine.Imaging              (PngWriter.Encode for captures, PngReader through the height-import command path)
...
KhaozEngine.Catalog.Sqlite    -> KhaozEngine.Sqlite               (SqliteStoreConnection, the shared unpooled open/gate/dispose lifecycle)
...
KhaozEngine.Catalog.AzureBlob -> KhaozEngine.Catalog              (IPackStore, IPackStorePruning, the content-address rule and FileSystemPackStore.RelativeKeyFor. NOTHING else, and never the authoring half)
KhaozEngine.Catalog.AzureBlob -> Azure.Storage.Blobs              (the SDK, declared HERE and nowhere below it, and no Azure.Identity: the caller hands over a built BlobContainerClient)
...
`KhaozEngine.Catalog.AzureBlob` follows the same opt-in rule as the two SQL providers: it is in NO umbrella and
a game references it explicitly, so nothing that merely reads content pulls an Azure SDK. Its one edge is on
`KhaozEngine.Catalog` and it never reaches the authoring half, because it implements a READ side seam and has
no business with a draft, a publish or a database. Inside the package the SDK is confined to the internal
`IBlobContainer` seam and its single adapter, which is what keeps `Azure.Storage.Blobs` out of every other
file: the store itself is ordinary code over five plain members, and the service's own shapes (a status code,
an `ETag` condition, a streaming download's declared length) stop at the adapter. The package's public surface
names one SDK type, the `BlobContainerClient` a caller hands to the constructor.
...
The seams the family owns are in the table above: `IPackStore` (four providers now, not one),
...
| Account registry | `../KhaozEngine.Accounts/IAccountStore.cs`, `AccountRecord.cs`, `AccountSignIn.cs`, `AccountStoreRules.cs` | `../KhaozEngine.Accounts/InMemoryAccountStore.cs`, `AccountBanStore.cs`, `../KhaozEngine.Accounts.Sqlite/SqliteAccountStore.cs`, `../KhaozEngine.Accounts.SqlServer/SqlServerAccountStore.cs`, conformance in `../KhaozEngine.Accounts.Tests/AccountStoreConformance*.cs` |
...
| Sign-in exchange | `../KhaozEngine.Identity.Exchange/IAuthExchangePolicy.cs`, `AuthExchangeOptions.cs`, `AuthExchangeOutcome.cs`, `AuthExchangeResult.cs` | `../KhaozEngine.Identity.Exchange/AuthExchange.cs`, `AuthAdmission.cs`, and the HTTP handler `../KhaozEngine.Identity.Exchange.AspNetCore/AuthExchangeEndpoints.cs`, `AuthExchangeHandler.cs`, `AuthExchangeHosting.cs`, `TrustedProxyNetworks.cs`, `AuthExchangeClientKey.cs` (ASP.NET Core) |
