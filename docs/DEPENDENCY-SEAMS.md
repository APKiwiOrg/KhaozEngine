...
| Tile actor behaviour | `KhaozEngine.TileWorld.Netcode` (`ITileActorBehaviour`, one `Decide` over a `TileActorContext` returning a `TileActorIntent`, plus `TileActorRandom`, the per-actor splitmix64 stream that lets an implementation stay stateless) | `TileWanderBehaviour` is the engine's own default (leash, chase, retaliate, wander, in that order), and a game replaces it wholesale rather than extending it. Nothing installs it: `TileActorHost.Behaviour` is null by default, so an actor with no behaviour stands where it was put. ONE instance is shared by every actor, exactly as a simulator is. The seam's line is drawn at exactly one place: an intent names a TILE or a TARGET and never a route, a step, a facing or a tick, so everything about HOW an actor moves stays inside the stepper both heads run and an actor can never move in a way a player could not | (no extra dep) |
