...
`Vortice.D3DCompiler` is reached from exactly ONE type, `Internal/D3D11Fxc`, which compiles emitted HLSL to DXBC
and reflects a vertex module's input signature. Everything else in the shader path (the target profiles, the
compile flags, the cache key, the disk cache and the holed-signature rule) names no Direct3D type at all and is
tested headlessly on macOS and Linux. One FXC call site is also what lets
