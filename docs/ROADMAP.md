...
Larger feature areas identified but not yet scheduled. Current released version: **4.10.0**.
...
### Current status (as of `5.14.0-experimental`)
...
- **5.x experimental line** — the custom-stack packages (`Render3D`, `Render2D`, ...) share a second version,
  `Directory.Build.props` `<KhaozEngine5xVersion>` (currently `5.14.0-experimental`), and release together
