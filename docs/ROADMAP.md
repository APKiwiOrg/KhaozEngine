...
Larger feature areas identified but not yet scheduled. Current released version: **7.11.0** (the shared
...
_(Historical snapshot below; current state is 7.11.0 - see CONSUMERS.md / CHANGELOG.md. The table captured
...
remain; the rest is productization + porting.** Shipped engine packages (shared version, now at 7.11.0):
...
The engine is now one MonoGame-free shared version line, currently 7.11.0 (see CONSUMERS.md / CHANGELOG.md).
