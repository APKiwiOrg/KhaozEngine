...
1. **The publish window is CLOSED, by a freeze and a re-proof.** Every publish the runner performs calls
   ...
   published. The marker rule is stated in full: the runner freezes only a draft it has just read as exactly
   its own plan, and it releases the marker on every attempt that froze and did not publish, including a
   cancelled token and a fault of a type this package does not report on, because a run that stops for an
   operator must not hand back a draft they can neither edit nor discard. What it releases is not always what
   it set. `FreezeDraftAsync` OVERWRITES and carries no identity, so a publisher that froze the same draft in
   the gap between the runner's read and the runner's own freeze has already lost its marker to the runner's.
   A rival RUNNER loses nothing by that, because its own re-proof under its own freeze refuses a contaminated
   draft exactly as this one did. The admin console's publish takes whatever is drafted with no plan proof at
   all, and that is residue 3 below.
...
1. An operator edit landing between the known-work proof and the discard it authorised is lost.
2. An operator edit on the SAME target under the SAME operation as one of the run's planned edits, written
   into the window where the run had seen no draft, is replaced by the run's own apply. A change set holds
   one pending intent per row, so the second write of a target takes the first one's place.
3. A CONSOLE publish that froze the same draft in the gap between the runner's read and the runner's own
   ...
   publish's own frozen edit set is then accepted, and that publish's commit deletes it unpublished. It is
   reachable only through the admin console, because a rival runner's own re-proof refuses the same
   contaminated draft.
...
keyboard at all. A game whose admin console writes under the same actor string as its upgrade runner weakens
every proof here, because the actor is the first half of each of them, so the upgrade actor must be dedicated
to the runner and used by nothing else.
