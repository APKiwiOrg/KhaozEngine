...
| M-W1 | Swapchain | `CAMetalLayer` configuration reproduced from the incumbent field for field, because it is visible only to a human: the layer adopt-or-create dance on the host view, `device`, `pixelFormat` from the sRGB request, `framebufferOnly = true`, and `drawableSize`. W1's lesson binds hardest where nothing in CI runs, and the Metal golden suite is headless. (**Corrected after gate 5**: `drawableSize` is the one field that is NOT reproduced. The incumbent's three source arms disagree with each other about points versus pixels, so field-for-field parity had no single field to reproduce, and the arm that multiplies by the backing scale is the right one. See the fifth addendum in 11.1 and [#605](https://github.com/APKiwiOrg/KhaozEngine/issues/605).) | Both, converged |
...
**The seat is now OCCUPIED. 2.3a is the measurement, it passed, and the filing it authorises is
https://github.com/APKiwiOrg/KhaozEngine/issues/604.**

### 2.3a MM6's result: the constraint is the incumbent's NUMBERING, and the rule as written is too broad

**Taken 2026-08-11 at row 17 (#583), and MM6 PASSES.** Measured on an Apple M2 Max under macOS 26, headless,
with the native backend and the incumbent stood up in ONE process against the same device, which is what
`GpuDeviceContext.CreateHeadless(GpuBackendKind)` exists for. The instrument is
`KhaozEngine.Render.Tests/Gpu/MetalTwoUniformBufferGpuTests.cs`.

**The shape of the probe, because the whole value of a measurement is that a reader can check it.** Each row
draws a full-screen triangle whose colour is composed from TWO uniform buffers and reads a texel back through
the seam's own `CopyTexture` and `Map`. The first buffer holds `(1, 0, 0, 1)`, the second `(0, 1, 0, 1)`, and
the shader writes `(A.r, B.g, A.g + B.r)`. So both buffers landing is YELLOW, the second reading all zero is
RED, the second aliasing the first is MAGENTA, the first aliasing the second is CYAN, and nothing bound at all
is BLACK. Five outcomes and five colours, which is row 14's discipline and the reason section 18's row 17 note
insists on a pixel: a probe whose only failure is black cannot tell a mis-bind from a pipeline that never
bound.

| Shape | `MetalNative` | `Metal`, the incumbent |
|---|---|---|
| Vertex stage reads two uniform buffers, set 0 and set 1 | `(1, 1, 0, 1)` both read | `(1, 1, 0, 1)` both read |
...
reproducing on this toolchain". That is exactly the confound the incumbent control was added to catch, and
catching it is what a control is for. The third shape is the one the seam doc's own last paragraph describes,
an earlier skinned pipeline whose vertex stage read the combined bone buffer alone and whose frame UBO sat
fragment-only in a second set, and it reproduces the failure on the incumbent, on this machine, today. So the
measurement has a live positive control rather than a remembered one.
...
error. **Nothing about Metal is involved in that at any point.**

**So the answer to the question that cost four diagnostic sessions is: the incumbent's numbering.** M-B1's
table binds where the emission put each argument, and where the two disagree it is the count that is wrong.
2.2b took the id join partly on the ground that (a) would make MM6 unreadable, and this is that reasoning
arriving: under the fallback this backend would have counted too, both hypotheses would have predicted the same
red, and the probe could not have separated them.

**AND THE RULE AS `docs/DEPENDENCY-SEAMS.md` WRITES IT IS TOO BROAD, which is a second finding worth as much as
the first.** That section says any pipeline reading more than one uniform buffer mis-binds, full stop. Two of
the three shapes above are two-uniform-buffer pipelines that bind correctly on the INCUMBENT today. What
mis-binds is narrower and has a rule: a STAGE that references fewer buffers than the declared array puts before
them. Every shipped program stays inside the safe region either way, which is why the over-broad rule has cost
nothing so far, and it is also why nobody noticed: the engine-wide fold-into-one discipline made the
distinction unobservable.

**This is the first program where the id join and the count disagree, and that is not a contradiction of
2.2b's evidence.** `MetalShaderIndexTableTests` asserts ZERO disagreements over the shipped corpus, 159 of 159,
and that assertion is untouched: it is a fact about the programs this engine ships, not about the arithmetic.
The third shape here is a constructed counter-example, which is 2.2b's "the day a shader changes" arriving in a
test rather than in a renderer.

**WHAT THIS AUTHORISES, AND WHAT IT DOES NOT.** M-B4 is unchanged and stays in force. No shader moved in row
17, and none may move on the strength of this. The Veldrid Metal leg still ships and still numbers by
declaration order, so a shader that spread its uniforms across sets would be correct on the native backend and
read zeros on the incumbent, which is the same argument 12.6 makes for keeping the sample-in-binding-order
discipline until that leg goes. The pass authorises exactly one thing, which 2.3 named in advance: FILING the
invariant's removal as its own work with its own gates on all three backends. That is
https://github.com/APKiwiOrg/KhaozEngine/issues/604, filed with the measurement in its body, triggered by the
closing act rather than by this phase, and it is MF3's issue.

**The caveats, so a later reader knows what was and was not measured.** One device, one macOS version, one
Veldrid and SPIRV-Cross pin, headless. Headless is the right instrument per 2.3, because the seam doc records
the constraint holding offscreen as well as windowed, and gate 5's windowed checklist still carries the
windowed class. The shapes measured are uniform buffers only: the seam doc also names a storage buffer
alongside a uniform buffer in the vertex stage, which is NOT measured here and belongs to #604's gates. And
anyone re-taking this measurement should run the THIRD shape first, because it is the only one of the three
that discriminates.

**One thing this measurement does NOT explain, said plainly rather than smoothed over.** The seam doc's
original vertex-stage report is a bone-palette ARRAY losing everything past element 0, which is a different
signature from a second buffer reading all zero and is not what the counting mechanism above predicts. The
first shape here is the closest thing to it that this row measured, and it came back clean on both backends,
under a Veldrid and SPIRV-Cross pin that is years newer than the report. So that signature is currently
UNREPRODUCED rather than explained, which is a narrowing rather than a refutation, and #604's gates carry it
...
genuinely left with no automated coverage is two things rather than the whole area: the six Cocoa selectors
that turn an `NSWindow` into a layer and a size, and whether anything appears on a screen. Both are gate 5's,
and MM7's real lesson survives intact for them, with one item added to that checklist:
[#605](https://github.com/APKiwiOrg/KhaozEngine/issues/605), the initial `drawableSize` being the content view's
frame in POINTS where a drawable size is in PIXELS.

**Fifth addendum, after gate 5: #605 is FIXED rather than reproduced, and the reason is that the incumbent
disagrees with itself.** The reproduction above was the right call at row 15 and is the wrong call now, and both
halves of that are worth stating because the reasoning is the reusable part.

*Why reproducing was right then.* Row 15's job was a backend swap with no rendering change, on the one surface
MM7 says nothing in CI can see. Correcting the arithmetic there would have been a resolution change smuggled
into a swap, and the whole point of holding the swap to field-for-field parity is that a difference nobody can
measure is a difference nobody can attribute. So it was reproduced, and
`MetalSwapchainPolicyTests.AHostViewFrameTruncatesRatherThanRounds` pinned the truncation as deliberate rather
than accidental.

*What changed.* `MTLSwapchain`'s constructor has THREE source arms, and they do not agree. The `NSWindow` and
`NSView` arms read `frame.size` and write it into `drawableSize` unchanged. The `UIView` arm multiplies by
`view.contentScaleFactor` first. A view frame is in points and a drawable size is in pixels on every one of
those platforms, so the UIView arm is right and the other two are wrong, on the incumbent's own terms. Once that
is the finding, "reproduce field for field" stops being a parity argument and becomes a request to reproduce a
DEFECT while the same file carries its fix twelve lines away. M-W1 reproduces DECISIONS. It has never covered
the incumbent's bugs, which is why M-W2 to M-W6 exist at all, and this is the fifth of them arriving late.

*What the fix is.* `-[NSWindow backingScaleFactor]` is read in `MetalLayerHost.Resolve`, right after the content
view answers non-nil, and the multiply happens in `MetalSwapchainPolicy.SizeOfHostView(frame, backingScale)`.
The scalar is read rather than the whole conversion being handed to `-[NSView convertRectToBacking:]`, which
would have produced the same pixels on a Mac and moved the one piece of this that CAN be asserted on Linux and
Windows into a selector that can be asserted nowhere. Truncation stays and now happens after the multiply, which
is what the UIView arm does and is the correct order: truncating first loses up to a whole pixel per point of
scale. A degenerate scale falls back to 1.0, the non-Retina identity, because `objc_msgSend` to nil answers zero
and a zero applied faithfully would configure a layer at nothing at all on a window whose points are perfectly
readable. That fallback is exactly the incumbent's behaviour, so the divergence is the smallest one available.

*What is measured and what is not.* The arithmetic, the truncation order, the saturation guard and the
degenerate-scale fallback are all device-free rows on every leg. The ABI is not new: the prototype is a bare
`double` return, which row 1's spike ran verbatim as `MsgSendDouble` and value-checked by reading `2.0` back off
`-setContentsScale:` on a real layer (3.1), so this is the one direction in the interop that was already
measured on the exact quantity it now reads. What no headless test can answer is the on-screen result, and gate
5's own reading is what makes that a small gap rather than a large one: the window was full-scale THERE, which
says the first framebuffer-resize callback lands before anything visible renders on that machine and that path.
So the honest claim is that this removes a wrong number from frame one, not that it fixes a visible defect
anybody saw. The confirmation still owed is one windowed run on a Retina display reading
...
**Addendum, at row 19: arming tier one is not free the way M-T7 says, and the correction has two measurements
behind it.** M-T7 calls `MTL_DEBUG_LAYER=1` "an environment variable with no install and no measurable cost at
this scale", and the COST is indeed nothing. What it has instead is a failure MODE, and #591 recorded it as a
property of `MTL_DEBUG_LAYER_ERROR_MODE=assert`, which reads as "do not set that variable and you are fine".
**Assert is the DEFAULT.** The first armed run of the whole assembly aborted the test host on the first
objection: 5956 of 6039 rows reported, then "Test Run Aborted", which is a gate that cannot pass silently and
also cannot tell you about the six thousand rows it never reached.

`MTL_DEBUG_LAYER_ERROR_MODE=nslog` was measured as the obvious alternative and is **REJECTED**: it stops the
abort and reports nothing at all to the captured stream, not even the layer's own "Metal API Validation
Enabled" banner, so the leg would upload an artifact named for a tier that neither failed nor testified. An
empty artifact under a true name is the failure mode #423 is about, arriving from the other direction.
...
- The validation-reader row asserts that a variable set in-process is tellable apart from one set at launch,
  which is unobservable in a process launched with it.
- **MM6's incumbent CONTROL reproduces the Veldrid Metal mis-binding on purpose**, and the layer's objection is
  `Fragment Function(main0): missing Buffer binding at index 0`. That is 2.3a's mechanism exactly: the
  incumbent writes the fragment's buffer at index 1 while the emitted function reads index 0. **Metal's own
...
**Addendum, at row 19: that cell was already satisfied, and it is not what the 284-failure run needed.** Both
assemblies that carry `[GpuFact]` (`KhaozEngine.Render.Tests` and `KhaozEngine.MapEditor.Tests`) already define
the collection, so the audit this row owed came back clean. What row 14 measured was a different failure with a
similar shape, and the difference is the whole lesson: the collection holds the classes that build a device
BESIDE the suite's own, which is the right membership on every leg where the suite runs on ANOTHER backend. On
the `metal-native` leg the suite runs on THIS backend, so every engine-wide device-building class builds one of
these devices, and `MetalCompletionHandler`'s four-slot registration table met the fifth concurrent one.
Measured on an Apple M2 Max, `KhaozEngine.Render.Tests` under `KE_GRAPHICS_BACKEND=metal-native`:

```
before, parallel     Failed: 284, Passed: 5755, Skipped: 0, Total: 6039    4m06s
before, serialised   Failed:   1, Passed: 6038, Skipped: 0, Total: 6039   23m12s
after,  parallel     Failed:   0, Passed: 6042, Skipped: 0, Total: 6042    4m37s
```

The AFTER row is the leg's exact shape rather than the before row's: `KE_GPU_TESTS=1`,
`KE_GRAPHICS_BACKEND=metal-native`, `KE_METAL_REQUIRED=1` and `MTL_DEBUG_LAYER=1`, which is what makes 0
skipped mean something (a dormant row throws there) and what makes the pass a statement about API validity
rather than only about pixels. The three extra rows are this row's own: the table's floor ratchet and the two
that assert the `KE_METAL_REQUIRED` refusal message off macOS.

Every one of the 284 is the same registration refusal, checked rather than assumed: a histogram over the run's
error messages is 283 of that exception plus one concurrency row reporting four threads failing with it.

**The alternative was PRICED rather than argued away**, because the cell's literal reading (put every
device-building class in the collection) is a real option and the reason to decline it has to be a number. A
fully serialised run of the same assembly on the same commit takes **23m12s against 4m06s parallel**, and the
collection is backend-agnostic, so that cost would land on the Windows and Linux legs too for a limit only this
backend has. That number is also the size of the thing this row was warned about: the regression column for
...
gates 1 and 2 and can read neither, because a hosted leg's first run is the push that merges it. So the three
edits the flip consists of are all deliberately absent from this row: the `ProbeOS` macOS arm still answers
`Metal`, `MetalNative` is still absent from `_windowCandidates`, and the two frame-cap sites still take the
conservative Metal arm. All three sites already carry the doc comment saying what they are waiting for, which
row 3 wrote and which is the reason a reader finding them unflipped is not left guessing.

**The SOAK BUILD is gate 4's, not row 19's, which is phase 3's ruling on the identical cell.** Section 18 lists
it in this row's scope and what that means is that nothing stands in its way any more: the backend creates a
windowed device, the counters this gate reads all reach a capture, `KE_GRAPHICS_BACKEND=metal-native` names it
on any Desktop head, and `GpuDeviceContext.CreateOrFallBack` reports `FallbackAfterFailure` in a log line so a
...
unmeasured. It stays open as the arithmetic question it always was. **(Closed after the gate: the initial size
now multiplies by `-[NSWindow backingScaleFactor]`, because the incumbent's own UIView arm does and its NSView
arm does not, so reproducing the NSView arm was reproducing a defect. The fifth addendum in 11.1 carries the
whole argument. The gate's reading above is unchanged and is what keeps the claim modest: this removes a wrong
number from frame one rather than fixing a defect anybody saw.)** #607, the single warmup crash, is an
...
**Owed against done, at row 19.** Several of these bullets were taken by the row that made them true rather
than waiting for this one, which is the right direction and leaves this list reading as though nothing had
happened. Each bullet below now says where it landed, so the sweep's value is the DIFFERENCE between owed and
done rather than a re-read of the whole list.

- `docs/DEPENDENCY-SEAMS.md`'s out-of-package backends section gains the THIRD instance of the inverted edge,
  and its GPU row's backend column becomes three native backends. (**DONE at row 19.** The section carries the
  Metal instance with the two things a reader would otherwise take from the wrong sibling: the package declares
  no third-party binding at all, which is a first for this program, and its platform boundary is the Direct3D 11
  apparatus rather than the Vulkan package's deliberate absence of one.)
- **`docs/DEPENDENCY-SEAMS.md`'s "ONE uniform buffer per pipeline" section gains MM6's RESULT**, and gains it
  ...
  behind it for the first time. (**DONE at row 17 rather than row 19**, because the measurement and the
  sentence it falsifies land together or the doc states a broad rule this repository has just measured as
  false. The section now carries the counting mechanism, the two shapes that bind correctly on the incumbent,
  the native backend's clean result, the unreproduced historical signature, and the rule kept with #604 as its
  removal.)
- `docs/USING-KHAOZENGINE.md` gains the backend-selection token and the `KE_METAL_*` variables. (**DONE at rows
  3, 15 and 16**, as each landed the thing it documents. Row 19 added only what gate 5 owns: the Mac vsync
  caveat now says which Metal it measured. `KE_METAL_REQUIRED` is deliberately NOT here, because it is a CI
  leg's variable read by tests rather than a consumer lever, which is where `KE_VULKAN_REQUIRED` lives too.)
- `docs/CROSS-PLATFORM.md`'s platform-to-backend mapping gains the native Metal leg. (**DONE at row 19**, and
  it took more than the mapping: the leg list, the trigger table, the artifact inventory and the six-legs count.
  One stale claim fell out of it, the Metal leg described as self-hosted a release after #552 moved it.)
- `GpuInterfaces.cs`'s `Begin` XML doc gains its Metal sentence, and the sentence that currently says the same
  ...
  gains its third arm (10.3). (**DONE at row 19**, all three. Rule 1's Metal sentence covers both
  implementations and now says that outright, because a sentence that happens to be true of two backends reads
  identically to one written to the API, which is the failure this comment exists to prevent.)
- `GpuDeviceCounters.cs`'s `BackpressureStallCount` doc gains the note that its second meaning is Vulkan's.
  (**DONE at row 19.** The useful half is the reading difference: a non-zero count on Metal is unambiguously a
  `KE_METAL_FRAMES_IN_FLIGHT` statement, where the same number on Vulkan folds two sources. The type's
  absent-is-not-zero paragraph was stale in the same way and said two native backends.)
- `GpuCapabilities.SupportsCompletionFences`'s doc names Veldrid's Metal mechanism and stays correct, and
  ...
  which implementation it describes. (**`SupportsCompletionFences` NOTHING OWED, confirmed at row 16** by
...
- `ModelRenderer.BeginModelPass`'s MRT-clear comment. (**DONE at row 19.** It now names the Veldrid Metal
  ...
  until the incumbent leg retires.)
- And both prior design docs get a corrected-in-place note in their own established style: phase 2's F2 and
  ...
  and Vulkan mapping onto each other is confirmed by 7.1. (**DONE at row 19, with one correction to this
  bullet.** Phase 2's shared-home statement is not its F2, which is the `Veldrid.SPIRV` shim: it is the
  qualified reuse bullet in its section 17, already corrected once on 2026-08-05. Both got a note, and F2 got
  its own, because 12.2 and section 19 answer it too and the answer is "not in this phase either".)
- `docs/INDEX.md`'s design table gains a row plus a status line, or this document is orphaned on arrival.
  (**Row DONE at row 1, status line rolled at every row since**, this one included.)
...
- **MF3.** The one-uniform-buffer-per-pipeline invariant's removal, IF MM6 passes. Its own work with its own
  ...
  named consumer that folded a second UBO into the frame UBO to work around it. **MM6 passed at row 17, so this
  is FILED with the measurement in its body: https://github.com/APKiwiOrg/KhaozEngine/issues/604, triggered by
  the closing act rather than by this phase (2.3a).**
