...
**Scope:** This plan covers the engine package + tests + `FileLogger` removal + `AppDataPaths` promotion + docs + the 3.1.0 release. The three consumer migrations (SpaceGame, Nullwake, Hardpoint) are **separate follow-on plans** written after 3.1.0 is packed to `local-feed`, because none of them can compile against the new API until the nupkg exists. See "Follow-on plans" at the end.
...
Expected: PASS (all of Task 4's sync tests still green + 7 async tests, including a re-entrant-flush deadlock guard and a log/flush-after-shutdown never-throw check).
...
> **Coordination gate (hard constraint):** these files are also touched by the `worktree-batch1-promote` work. This release is **3.1.0**, folded to `main` AHEAD of batch1 (which then rebases to 3.2.0 on top). Before doing this task, run `git -C /Users/antonio/KhaozEngine fetch` and confirm `origin/main` is still 3.0.0 with no `v3.1.0` tag (nobody else has claimed 3.1.0) and no other chat is mid-release. Never pack/overwrite the shared `local-feed` concurrently. After release, hand off to the batch1 chat so it adopts 3.2.0.
...
git fetch origin
git merge origin/main          # origin/main is 3.0.0; no-op fast-forward unless something else landed
...
- **BREAKING (shipped as a minor)**: `FileLogger` is removed; consumers move to `Log`/`LogManager`. The
  ...
  is kept. All packages to 3.1.0.
```

- [ ] **Step 4: Update `docs/CONSUMERS.md` version line + matrix**

Change `**Engine current version:** `3.0.0`` to `3.1.0`, bump the Diagnostics column / engine-version references as appropriate, and update `_Last verified_` to today. (Consumer pins stay where they are until each migration plan runs.)
...
ls /Users/antonio/KhaozEngine/local-feed/KhaozEngine.Diagnostics.3.1.0.nupkg   # expected: present
```
...
git commit -m "Release KhaozEngine 3.1.0 (logging service replaces FileLogger)"
```
...
Expected: PASS. Confirm `v3.1.0` is pushed and CI publishes to GitHub Packages on the tag.
...
- Replace FileLogger (breaking removal, shipped as minor 3.1.0) → Task 11, 13. ✓
